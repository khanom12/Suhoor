...
struct DefaultAlarmConfig: Codable, Equatable, Sendable {
    ...
    static let legacySuhoorFactoryDefault = DefaultAlarmConfig(
...
extension DefaultAlarmConfig {
    var usesLegacyFactoryWakeDefault: Bool {
        let legacy = Self.legacySuhoorFactoryDefault
        return suhoorEnabledDefault == legacy.suhoorEnabledDefault
            && defaultWakeState == legacy.defaultWakeState
            && defaultWakeAnchorType == legacy.defaultWakeAnchorType
            && defaultWakeDeltaMinutes == legacy.defaultWakeDeltaMinutes
            && defaultLatestWakeCapMinutesFromMidnight == legacy.defaultLatestWakeCapMinutesFromMidnight
            && defaultSuhoorTimeMode == legacy.defaultSuhoorTimeMode
            && defaultSuhoorOffsetMinutes == legacy.defaultSuhoorOffsetMinutes
    }

    mutating func migrateLegacyFactoryWakeDefaultToSubhDefault() -> Bool {
        guard usesPreSubhFajrStartWakeDefault else { return false }
        defaultWakeState = Self.default.defaultWakeState
        defaultWakeAnchorType = Self.default.defaultWakeAnchorType
        defaultWakeDeltaMinutes = Self.default.defaultWakeDeltaMinutes
        defaultLatestWakeCapMinutesFromMidnight = Self.default.defaultLatestWakeCapMinutesFromMidnight
        defaultSuhoorTimeMode = Self.default.defaultSuhoorTimeMode
        defaultSuhoorOffsetMinutes = Self.default.defaultSuhoorOffsetMinutes
        return true
    }

    private var usesPreSubhFajrStartWakeDefault: Bool {
        let delta = max(0, defaultWakeDeltaMinutes)
        return usesLegacyFactoryWakeDefault
            || (
                defaultWakeState == .preFajr
                    && normalizedDefaultWakeAnchorType == .fajrStart
                    && defaultSuhoorTimeMode == .relativeToFajrMinusMinutes
                    && delta == max(0, defaultSuhoorOffsetMinutes)
                    && [30, 45].contains(delta)
                    && defaultLatestWakeCapMinutesFromMidnight == nil
            )
    }
