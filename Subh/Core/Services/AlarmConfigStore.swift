...
final class AlarmConfigStore: ObservableObject {
    private static let latestMigrationVersion = 5
    ...
    private func performMigrationIfNeeded(legacySettings: AppSettings?) {
        ...
        if currentVersion < 4 {
            if defaults.migrateLegacyFactoryWakeDefaultToSubhDefault() {
                persistDefaultsImmediately()
            }
            currentVersion = 4
        }
    ...
    private func persistDefaultsImmediately() {
