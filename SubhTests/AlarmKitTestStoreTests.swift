...
@Suite
@MainActor
struct AlarmKitTestStoreTests {
    @Test
    func settingsStoreRecoversPersistedValues() throws {
        let suiteName = "SuhoorTests.AlarmKitTestSettingsStore"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let storageKey = "suhoor.alarmKitTestSettings"

        let missingStore = AlarmKitTestSettingsStore(defaults: defaults)
        #expect(missingStore.settings == .default)

        let persisted = AlarmKitTestSettings(
            isEnabled: true,
            suhoorOffsetSeconds: 15,
            reminderOffsetSeconds: 30,
            adhanOffsetSeconds: 45,
            testRunId: UUID()
        )
        defaults.set(try JSONEncoder().encode(persisted), forKey: storageKey)

        let validStore = AlarmKitTestSettingsStore(defaults: defaults)
        #expect(validStore.settings == persisted)

        defaults.set(Data("not-json".utf8), forKey: storageKey)

        let recoveredStore = AlarmKitTestSettingsStore(defaults: defaults)
        #expect(recoveredStore.settings == .default)

        let recoveredData = try #require(defaults.data(forKey: storageKey))
        let recoveredSettings = try JSONDecoder().decode(AlarmKitTestSettings.self, from: recoveredData)
        #expect(recoveredSettings == .default)
    }

    @Test
    func runStoreRecoversPersistedValues() throws {
        let suiteName = "SuhoorTests.AlarmKitTestRunStore"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let storageKey = "suhoor.alarmKitTestRun"
        let store = AlarmKitTestRunStore(defaults: defaults)

        #expect(store.load() == nil)

        let runState = AlarmKitTestRunState(
            testRunId: UUID(),
            baseNow: Date(timeIntervalSince1970: 1_772_409_600),
            suhoorDate: Date(timeIntervalSince1970: 1_772_409_660),
            reminderDate: Date(timeIntervalSince1970: 1_772_409_720),
            adhanDate: Date(timeIntervalSince1970: 1_772_409_780)
        )
        store.save(runState)
        #expect(store.load() == runState)
