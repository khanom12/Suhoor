...
struct SubhTests {
    ...
    @Test
    func eventTimesUseResolverForWakeAndFajrOffsetForReminder() {
        var calendar = Calendar(identifier: .gregorian)
        calendar.timeZone = TimeZone(secondsFromGMT: 0) ?? .current
        let fajr = calendar.date(from: DateComponents(year: 2026, month: 2, day: 20, hour: 5, minute: 30))
        let fajrEnd = calendar.date(from: DateComponents(year: 2026, month: 2, day: 20, hour: 6, minute: 15))
        let maghrib = calendar.date(from: DateComponents(year: 2026, month: 2, day: 20, hour: 18, minute: 0))
        #expect(fajr != nil)
        #expect(fajrEnd != nil)
        #expect(maghrib != nil)
        guard let fajr, let fajrEnd, let maghrib else { return }

        let prayerWindow = DailyPrayerWindow(date: fajr, fajrStart: fajr, fajrEnd: fajrEnd, maghrib: maghrib)
        let wakeRule = MorningWakeRule(state: .inFajr, anchorType: .fajrEnd, deltaMinutes: 30)
        ...
        let wake = MorningScheduleResolver.resolveWakeTime(
            day: fajr,
            prayerWindow: prayerWindow,
            anchor: anchor,
            wakeRule: wakeRule,
    ...
    func dstBoundaryMaintainsOffsets() {
        ...
        let fajrEnd = calendar.date(byAdding: .minute, value: 45, to: fajr)
        let maghrib = calendar.date(from: DateComponents(year: 2026, month: 3, day: 8, hour: 18, minute: 0))
        #expect(fajrEnd != nil)
        #expect(maghrib != nil)
        guard let fajrEnd, let maghrib else { return }
        let prayerWindow = DailyPrayerWindow(date: fajr, fajrStart: fajr, fajrEnd: fajrEnd, maghrib: maghrib)
        let wakeRule = MorningWakeRule(state: .inFajr, anchorType: .fajrEnd, deltaMinutes: 30)
        let anchor = MorningScheduleResolver.resolveWakeAnchor(
            prayerWindow: prayerWindow,
