...
struct AlarmDayDetailView: View {
    ...
    private var configurationList: some View {
        List {
            ...
            if !sourceProvenances.isEmpty {
                Section {
                    ForEach(sourceProvenances, id: \.id) { provenance in
                        VStack(alignment: .leading, spacing: 4) {
                            Text(provenance.label)
                                .foregroundStyle(.primary)
                            if provenance.isExplicitOneOff {
                                Text("This date was added directly.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            } else {
                                Text("This date is generated by a saved range or recurring source.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
                ...
                Section {
                    ...
                    if !stoppableProvenances.isEmpty {
                        ...
                        ForEach(stoppableProvenances, id: \.id) { provenance in
                            Button(provenance.stopSeriesLabel ?? "Stop this series", role: .destructive) {
                                Task {
