...
struct AlarmTagFilter: Equatable {
    ...
    var selectedItems: [AlarmTagFilterItem] {
        var items: [AlarmTagFilterItem] = []
        if let primaryIntent {
            items.append(.primary(primaryIntent))
        }
        items.append(contentsOf: FastIntentEngine.displaySecondaryTags(secondaryTags).map(AlarmTagFilterItem.secondary))
        return items
    }
...
enum AlarmTagFilterItem: Identifiable, Hashable {
