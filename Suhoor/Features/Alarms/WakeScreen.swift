...
struct WakeScreen: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: Binding(
            ...
            AlarmTagFilterSheet(filter: Binding(
    ...
    private func refreshListSnapshot(animated: Bool) {
        ...
        let update = {
            let result = WakeListSnapshotBuilder.build(
                wakeSnapshot: scheduleManager.wakeSurfaceSnapshot,
                tagFilter: viewState.tagFilter,
                pinnedEntryIDs: viewState.pinnedNextWakeEntryIDs,
                totalScheduledCount: totalScheduledCount(for:),
                rollingHijriMonths: { scheduleManager.rollingHijriMonths(count: 12, timeZone: .current) },
                monthPreview: { yearMonth in
                ...
                cachedMonthEntries: { key in
                    scheduleManager.cachedMonthEntries(for: key)
                }
            )
    ...
    private func totalScheduledCount(for key: HijriMonthKey) -> Int {
