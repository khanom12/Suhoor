# wake-session-execution Specification

## Purpose
Define Subh's core/free Wake Session execution lifecycle, deterministic Wake Checks, local current-morning MorningLogs, Quiet active-session cancellation, and confirmation semantics. This spec keeps wake execution attached to the one Fajr-centered morning engine while preserving the separation between platform alarm events, awake confirmation, prayer confirmation, fasting intent, fast completion, and Quiet Morning.
