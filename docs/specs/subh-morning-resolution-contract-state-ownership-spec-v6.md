# Subh Morning Resolution Contract and State Ownership Specification v6 — June 1 CTA, Logging, and Early-Awake State Ownership Reconciliation

| Field | Value |
| --- | --- |
| Canonical filename | `subh-morning-resolution-contract-state-ownership-spec-v6.md` |
| Version | 6 |
...
Version 5 aligns the resolver with the May 31 morning-state framework: visible selector order, Today/Tomorrow Morning labels, Next 7 layout needs, wake-session boundary fields, slider-activated Fajr wake checks after Suhoor after Suhoor, and late Fajr logging inside the context-card action area.
...
| Suhoor acknowledged, no post-Suhoor Fajr slider activation requested | Suhoor/Fajr phase | fajrStartEventOnly | Fajr-start event only |
...
Fajr-start event           = default single event when Fajr begins after Suhoor
Fajr wake acknowledgement   = user is awake for Fajr after Fajr begins or after Fajr wake session fires
Fajr prayer completion      = user reports praying Fajr
Fast completion             = user reports after Maghrib that the fast was completed
```

For a Suhoor-selected morning:

1. **I’m Awake for Suhoor** records `suhoorWakeOutcome`.
2. Confirmed **I’m Already Awake for Suhoor** records `suhoorWakeOutcome = awake_early` and silences Suhoor attempts.
3. Fajr-beginning adhan/event remains by default after Suhoor wake resolution.
4. A full Fajr wake-check session is not automatic.
5. If the user commits a later Fajr slider value after Suhoor, create/activate a normal Fajr wake session.
6. **I’m Awake for Fajr** records `fajrWakeOutcome`.
7. **I Prayed Fajr** records Fajr prayer completion separately.
8. Fast completion is resolved after Maghrib through compact check/X prompts when eligible.

For a Fajr-selected morning:

1. confirmed **I’m Already Awake for Fajr** silences Fajr adhan/alarm/checks and records early Fajr wake;
2. ordinary system dismissal without explicit awake does not record Fajr wake;
3. **I’m Awake for Fajr** records `fajrWakeOutcome` directly.

## 12. Late Fajr prompt resolution

After Fajr end, the Hero switches to the next relevant morning. If Fajr prayer completion is not logged, resolver may expose a late prompt inside the context-card action area.
...
10. System dismissal without explicit awake confirmation produces an attempt-dismissed record, not wake acknowledgement; the resolver exposes the next pending wake-check time when one exists.
...
## June 1 Addendum: New Resolved Outputs

Morning Resolution must expose enough state for the Hero and context card to render the new CTA model.

Required resolved outputs include:

| Output | Meaning |
| --- | --- |
| `nextPendingWakeAttemptTime` | Earliest scheduled/pending future attempt in the active wake session. |
| `activeWakeCta` | `imAwakeForSuhoor`, `imAwakeForFajr`, or none. |
| `contextActionRows` | Ordered compact action rows for early-awake, Fajr logging, fast logging, and unresolved check-ins. |
| `contextActionOverflowCount` | Number of hidden rows when collapsed. |
| `fajrPrayerPromptState` | hidden / inWindowAvailable / lateSameDay / lateYesterday / answered / expiredUnresolved. |
| `fastPromptState` | hidden / sameDayAfterMaghrib / yesterday / answered / expiredUnresolved. |
| `earlyAwakeAction` | `alreadyAwakeForSuhoor`, `alreadyAwakeForFajr`, or none. |
| `postSuhoorFajrState` | none / defaultFajrBeginningEvent / sliderActivatedWakeSession. |

Resolution must not infer missed prayer or missed fast from silence.
