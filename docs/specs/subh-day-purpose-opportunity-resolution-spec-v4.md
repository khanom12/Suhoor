# Subh Day Purpose, Opportunity, Intention, Outcome, and Analytics Credit Specification v4 — June 1 Fajr/Fast Logging and Qada-Candidate Reconciliation

| Field | Value |
| --- | --- |
| Canonical filename | `subh-day-purpose-opportunity-resolution-spec-v4.md` |
| Version | 4 |
...
If Fajr prayer completion is logged after Fajr end through the prompt inside the context-card action area, analytics should credit it as late Fajr logging for the previous relevant morning.
...
## June 1 Addendum: Fast Completion Eligibility and Qada-Candidate Logic

Fast completion prompts appear after Maghrib when either:

1. the user selected Suhoor for that morning; or
2. the date is in Ramadan.

They do not appear merely because the date is a Monday, Thursday, White Day, or other optional fasting opportunity unless the user selected Suhoor.

Compact fast prompt states:

| Response | Meaning |
| --- | --- |
| ✓ | Fast completed. |
| ✕ | Fast not completed. |
| No response | Unrecorded/unresolved. |

Ramadan ✕ creates future Qada fast relevance. Optional fast ✕ supports statistics and encouragement but does not create the same Qada fast requirement.

Fajr ✕ from late or historical logging creates future Qada Fajr relevance. Expired unresolved prompts do not create Qada candidates.
