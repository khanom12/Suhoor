...
| Field | Value |
| --- | --- |
| Canonical filename | `subh-planning-horizon-day-resolution-intention-anchoring-spec-v2.md` |
| Version | 2 |
| Spec status | Product / design / engineering draft; aligned Desktop working spec |
| Supersedes | None recorded in the active Desktop set |
| Related specs | `00-subh-spec-index-v1.md`, `subh-morning-resolution-contract-state-ownership-spec-v2.md`, `subh-day-purpose-opportunity-resolution-spec-v1.md`, `subh-alarm-delivery-schedule-reliability-spec-v2.md`, `subh-quick-wake-mode-intent-mutation-contract-v1.md` |
...
- Generated-vs-stored doctrine for browsed months, Next 10, and future edits.
...
- Display horizons such as Next 10 and month browsing may show fasting opportunities, but opportunity-only dates must not become durable Suhoor intentions.
...
- how month browsing and Next 10 views relate to the active schedule;
...
| Month browsing and Next 10 generation policy | Planning Horizon and Intention Anchoring | Views are generated from the canonical resolver and may be cached temporarily. |
...
- **Display horizon** means what the user can see, such as Next 10 or a month view.
...
2. **Subh can show more than it needs to schedule.**
   The next 10 mornings or a future month can be visible without all of those days being active alarms.
...
- Do not hard-code the assumption that Subh only ever supports 10 days.
...
|---|---|---|
| Home display range | Next 10 mornings | Configurable, likely still next 10 |
...
- “Turn off next alarm” is not the same as “turn off every alarm for the next 10 days.”
...
- Next 10 rows are not automatically scheduled just because they are visible;
...
A browsed month, a displayed Next 10 row, or a stored future observance intention is not platform-scheduled merely because it exists.
...
The next 10 days can be resolved and shown. A future month can be browsed. But Subh should only activate events inside the canonical active scheduled window.
...
## 12. Next 10 Mornings Behavior

### 12.1 Purpose

The Next 10 Mornings card is a near-term view of the user's upcoming Fajr-centered mornings.

It should answer:

- What is coming up?
- Which mornings are ordinary?
- Which mornings have observance opportunities?
- Which mornings have user intentions?
- Which morning is operationally next?
- Which morning needs review?

### 12.2 Rolling window

The Next 10 Mornings view should behave like a rolling window.

Each day:

- the completed/past morning drops out,
- the remaining mornings shift forward,
- a new morning enters at the end,
- and all displayed mornings are re-resolved using current settings.

### 12.3 What each card should show
...
### 12.4 User changes in Next 10

If the user changes a day in Next 10, Subh must store the change as an anchored intention or override.
...
### 12.5 Default cards are generated, not stored

A day appearing in the Next 10 card does not mean Subh has permanently stored a record for that day.
...
- Ensure the change appears later in Next 10 when the date approaches.
...
Given a user opens the next 10 mornings and makes no edits, Subh should not create durable day-specific records for each default day.
...
Given a day appears in Next 10 or month browsing, Subh should not schedule a platform delivery for that day unless the canonical active window explicitly includes a resolver-materialized event for that date.
...
2. The next 10 mornings are generated for display from current rules.
...
#### Scenario: Next 10 shows future mornings

- GIVEN Next 10 displays ten mornings
...
- GIVEN the user opens Next 10 or a calendar month
