...
| Field | Value |
| --- | --- |
| Canonical filename | `subh-morning-hero-item-spec-v14.md` |
| Version | 14 |
| Spec status | Product / implementation direction; canonical Desktop working spec |
| Supersedes | Archive/Morning_Hero_Item_Specification_v13.md |
| Related specs | `00-subh-spec-index-v1.md`, `subh-morning-resolution-contract-state-ownership-spec-v2.md`, `subh-quick-wake-mode-intent-mutation-contract-v1.md`, `subh-early-worship-boundary-spec-v1.md`, `subh-next-10-mornings-wake-forecast-spec-v4.md`, `subh-weekly-fajrcast-card-spec-v13.md` |
...
## 0. v14 source-of-truth alignment

This v14 revision preserves the v13 visual, layout, animation, Dynamic Type, accessibility, and Home-surface behavior unless explicitly clarified here. The purpose of this revision is to align the Morning Hero with:

1. `00-subh-spec-index-v1.md`;
2. `subh-mvp-interaction-inventory-v3.md`;
3. `subh-quick-wake-mode-intent-mutation-contract-v1.md`;
4. the parent `subh-morning-resolution-contract-state-ownership-spec-v2.md`.
...
The Gregorian + Hijri date row is intentionally hidden in v14. Date data may remain resolved by the data layer and may return in a later iteration, but the visible top row for this version is the location line.
...
The date line is not part of the visible hero stack in v14. This is a temporary design decision; the resolved Gregorian and Hijri date rules are retained for future reactivation.

## 4. Current screenshot baseline

The earlier implementation visually showed a Home hero selector with `Fast | Fajr | Quiet`, while the detail screen used `Early | Fajr | Quiet`. v14 aligns both surfaces to the same mode labels:
...
Pre-Fajr + Other early worship example:
...
Main changes retained and clarified in v14:
...
25. Retain the full Gregorian + Hijri month-name date rules for possible future reactivation, but do not allocate visible space for the hidden date row in v14.
...
34. v14 formally routes all quick-mode, Pre-Fajr-intention, fasting-intention, and wake-adjustment changes through the shared Quick Wake Mode and Intent Mutation Contract.
...
The Gregorian + Hijri date line is hidden in v14 and must not reserve a blank row in the visible hero stack.
...
The Gregorian + Hijri date line is not visible in v14.
...
The relation/status line is the final informational text row in the v14 hero. The quick wake-state selector sits below it.
...
Visual aesthetic rules remain unchanged from v12: the selector must match the translucent, premium liquid-glass language used by the Weekly Fajrcast card and the Next 10 Mornings card, and it must not use a separate opaque gray segmented-control style.
...
| Hidden/reserved date line | 17 | regular, secondary opacity; not visible in v14 |
...
- In v14, `visibleTopLineMode` should be `location` by default.
...
- `dateLineVisible` should be `false` in v14.
...
- `quickWakeStateSelectorVisible` is `true` in the v14 hero unless the app is in a loading, missing-data, or unsupported state where mode changes cannot be performed truthfully.
...
- `relationLinePosition` is `aboveQuickWakeStateSelector` in v14.
...
- The hidden date row is not required in the visible summary in v14, but resolved date text may be included in accessibility if the product chooses to preserve date context.
...
- The hidden date row should not receive a visible loading placeholder in v14.
...
- Keep the visible date line hidden in v14.
...
These should be treated as v14 locked requirements:
...
6. The Gregorian + Hijri date line is hidden temporarily in v14.
...
15. The quick wake-state selector is the final base hero row in v14; conditional Pre-Fajr intention controls may appear below it when Pre-Fajr is selected.
...
- [ ] Relative-label-to-primary-row spacing uses the reduced v3/v14 gap.
...
It should be calmer and simpler than the Weekly Fajrcast card, but just as truthful. In v14, the top of the hero prioritizes the location used for the Fajr calculation rather than the date. This makes the resolved prayer-time context immediately visible before the user reads the target morning or wake state.

The v14 composition starts with the location line, then the relative day, then the primary wake time or quiet-state text. The compact wake-boundary visual appears below the primary row when eligible. On default Fajr mornings, it shows the Fajr begins → Fajr ends window with endpoint circles at both ends and the alarm icon positioned along the bar. On Pre-Fajr, Tahajjud-only, or Other early worship mornings, it shows the final-third start → Fajr begins window with a vertical line/tick at the left boundary, an endpoint circle at Fajr begins, and the alarm icon positioned along the bar. In Quiet mode, it keeps the Fajr begins → Fajr ends visual as static information with no wake marker and no draggable handle.
...
## 25. v14 non-removal guardrail

This v14 update is a reconciliation patch, not a visual redesign. Do not remove v13 layout, Liquid Glass treatment, selector animation, wake-boundary visual behavior, hidden date-line policy, dynamic-type rules, accessibility rules, Next 10 collapsed note, or Weekly Fajrcast host note unless a later named spec explicitly supersedes them.

The only behavior-level additions in v14 are the shared mutation-contract alignment, preservation of `Other early worship`, distinction from `Other fast`, immediate-commit clarification for Home Hero actions, and Quiet-versus-delivery-failure separation.
