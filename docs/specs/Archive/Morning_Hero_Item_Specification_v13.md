# Morning Hero Item Specification — v1.3 Codex-Ready Draft
...
The Gregorian + Hijri date row is intentionally hidden in v1.3. Date data may remain resolved by the data layer and may return in a later iteration, but the visible top row for this version is the location line.
...
The date line is not part of the visible hero stack in v1.3. This is a temporary design decision; the resolved Gregorian and Hijri date rules are retained for future reactivation.

## 4. Current screenshot baseline

The earlier implementation visually showed a Home hero selector with `Fast | Fajr | Quiet`, while the detail screen used `Early | Fajr | Quiet`. v1.3 aligns both surfaces to the same mode labels:
...
[ Tahajjud only selected | Fasting ]
```

Pre-Fajr + Fasting example:
...
[ Pre-Fajr selected | Fajr | Quiet ]
[ Tahajjud only | Fasting selected ]
...
[ Pre-Fajr | Fajr | Quiet selected ]
```

Main changes in v1.3:

1. Align the Home hero and Alarm Detailed View mode labels to `Pre-Fajr | Fajr | Quiet`.
...
7. When `Pre-Fajr` is selected outside Ramadan, the Home hero shows an intention selector with `Tahajjud only` and `Fasting`.
...
16. The quick wake-state selector changes the primary wake row, range visual, and relation/status line through the shared wake-state resolver; the hero view must not create or schedule alarms directly.
...
19. Use the early-worship adjuster for selected Pre-Fajr, fasting, Ramadan, Qada/custom fast, and Tahajjud-only mornings whose wake time is between the final-third start and Fajr begins.
...
25. Retain the full Gregorian + Hijri month-name date rules for possible future reactivation, but do not allocate visible space for the hidden date row in v1.3.
26. The `Pre-Fajr | Fajr | Quiet` selector must use the same translucent liquid-glass surface language as the Next 10 Mornings card, not an opaque or frosted gray segmented-control treatment.
...
32. On Home, `Next 10 Mornings` appears collapsed by default with its header visible.
33. The Weekly Fajrcast remains as currently designed and is not collapsed by this revision.
...
- **Pre-Fajr intention:** when `Pre-Fajr` is selected, the user’s intention is either `Tahajjud only` or `Fasting`.
- **Fasting intention:** when `Pre-Fajr` + `Fasting` is selected, the app resolves the applicable fasting intention: Ramadan, opportunity-based Sunnah fasts, Voluntary, Qada, Vow, Kaffarah, or Other, depending on date and user override rules.
- **Default Fajr window:** the Fajr begins → Fajr ends interval used for ordinary/default Fajr mornings.
- **Early-worship window:** the final-third start → Fajr begins interval used for Pre-Fajr, fasting, Ramadan, Qada/custom fast, and Tahajjud-only mornings.
...
Quick-select state changes are owned by the shared wake-state / alarm-state resolver. The hero emits a user intent such as `selectPreFajr`, `selectFajr`, or `selectQuiet`; the resolver returns the updated snapshot and coordinates persistence/scheduling through the single alarm-state pipeline.

Pre-Fajr intention changes and fasting-intention changes must also route through the shared resolver or the same date-specific intent pipeline. The hero must not directly create fasting plans, adhan events, or alarms locally.
...
7. **Pre-Fajr intention selector:** `Tahajjud only | Fasting`
...
{quickWakeStateSelector: Pre-Fajr | Fajr | Quiet}
...
The Gregorian + Hijri date line is hidden in v1.3 and must not reserve a blank row in the visible hero stack.
...
The Gregorian + Hijri date line is not visible in v1.3.
...
- The date line keeps the same visual font style it had in v0.1/v0.2/v0.3: same base size, weight, and secondary opacity.
...
- The relative label keeps the same visual font style it had in v0.1 through v0.3. Moving it below the location line must not reduce its intended prominence.
...
#### 7.5.2 Early-worship mode: fasting and Tahajjud mornings

Use this mode for mornings with intended fasting, Ramadan fasting support, Qada/custom fast support, intended Tahajjud-only, .
...
- Use `interactiveEarlyWorshipWindow` when the resolved day has selected `Pre-Fajr`, intended fasting, or intended Tahajjud-only and the active wake time or planned wake anchor is inside the final-third start → Fajr begins window.
...
The relation/status line is the final informational text row in the v1.3 hero. The quick wake-state selector sits below it.
...
- Use these patterns only when the resolved day has selected `Pre-Fajr`, intended fasting, or intended Tahajjud-only and the active wake time is inside the final-third start → Fajr begins window.
...
Visual aesthetic rules remain unchanged from v1.2: the selector must match the translucent, premium liquid-glass language used by the Weekly Fajrcast card and the Next 10 Mornings card, and it must not use a separate opaque gray segmented-control style.
...
- Outside Ramadan, the user may change the Pre-Fajr intention to `Fasting` when fasting is available.
- On Eid days, fasting is unavailable and selecting `Pre-Fajr` lands on `Tahajjud only`.
- During Ramadan, selecting `Pre-Fajr` restores locked `Fasting` + `Ramadan fast`.
- Selecting `Fajr` clears the active Pre-Fajr wake mode for the target morning unless another explicit calendar-locked state owns that day.
...
- Selecting `Quiet` suppresses the target morning's active wake alarm.
...
- Quick wake-state selector changes must go through the shared wake-state / alarm-state resolver. The hero view emits an intent and receives an updated snapshot; it must not directly create, cancel, or schedule alarms.
- If persistence or scheduling fails after a quick-select change, restore the previous resolved state and use the app's standard non-disruptive error treatment.
...
- Show the Pre-Fajr intention control only when `Pre-Fajr` is selected.
- Hide the Pre-Fajr intention control when `Fajr` or `Quiet` is selected.
- During Ramadan, the intention control is locked to `Fasting` or hidden with clear locked Ramadan state; the user cannot change it to `Tahajjud only`.
- On Eid days, `Fasting` is unavailable and the control resolves to `Tahajjud only`.
- Do not include `Other` in the Home hero intention control.
- Do not include `Fasting + Tahajjud` as a combined Home hero intention.

#### 7.7.2 Fasting-intention behavior in the Home hero

When the user selects `Pre-Fajr` + `Fasting`, show the fasting intention state.
...
The currently approved behavior is the selector movement inside the `Pre-Fajr | Fajr | Quiet` container. Preserve that sliding selected-state behavior. The refinements below constrain the rest of the hero so text, time, and adjuster transitions stay stable and understandable.
...
- `Next 10 Mornings` appears collapsed by default below the hero.
- The `NEXT 10 MORNINGS` header remains visible in the collapsed state.
- The user can expand the card to reveal the ten rows.
...
- Weekly Fajrcast remains as currently designed.
- Weekly Fajrcast is not collapsed by this revision.
- This revision does not remove or redesign the current Weekly Fajrcast interaction.
...
- Use one deliberate translucent liquid-glass segmented pill only for the `Pre-Fajr | Fajr | Quiet` quick wake-state selector.
- The selector must feel glassy and integrated with the background, matching the Next 10 Mornings card surface language rather than an opaque or frosted gray segmented-control treatment.
...
| Hidden/reserved date line | 17 | regular, secondary opacity; not visible in v1.3 |
...
| Quick wake-state selector pill | visual token | same liquid-glass surface family as the Next 10 Mornings card; three equal logical segments; selected highlight moves smoothly; 44 px/pt effective hit target per segment |
...
On intended fasting, Ramadan fasting, Qada/custom fasting, or Tahajjud-only mornings, show the early-worship adjuster instead of hiding the visual row.
...
- Use the early-worship adjuster only when the resolved day has selected `Pre-Fajr`, intended fasting, or intended Tahajjud-only.
...
- Fasting opportunity tags alone do not activate this visual. The day must be selected as Pre-Fajr or resolved as intended fasting, Ramadan fasting support, Qada/custom fasting, intended Tahajjud-only wake.
...
- Use the early-worship adjuster for intended fasting, `Pre-Fajr` quick-select mode, or intended Tahajjud-only mornings when final-third start and Fajr begins are available.
...
- preFajrIntention tahajjudOnly | fasting | lockedRamadanFasting | unavailable optional
- preFajrIntentionSelectionSource defaultTahajjudOnly | userSelected | ramadanLocked | eidFastingUnavailable | unavailable optional
...
- In v1.3, `visibleTopLineMode` should be `location` by default.
...
- `dateLineVisible` should be `false` in v1.3.
...
- `selectedQuickWakeMode = quiet` when the target morning is selected as Quiet or resolved as quiet/no-wake for this surface.
- `quickWakeStateSelectorVisible` is `true` in the v1.3 hero unless the app is in a loading, missing-data, or unsupported state where mode changes cannot be performed truthfully.
...
- Quick-select actions must be routed to the shared wake-state / alarm-state resolver. The hero renderer must not create, cancel, or schedule alarms locally.
- Selecting `Pre-Fajr` should produce an active wake default of 30 min before Fajr begins. Outside Ramadan the default Pre-Fajr intention is `Tahajjud only`; during Ramadan the locked intention is `Fasting` with `Ramadan fast`.
- Selecting `Fajr` should produce an active wake default of 30 min before Fajr ends unless a stronger existing user adjustment is deliberately preserved by the resolver.
- Selecting `Quiet` should suppress the active wake alarm and set `wakeDisplayText` to `Quiet mode`.
...
- `wakeBoundaryReason` should come from the same resolver that determines whether the day is a default Fajr morning, selected Pre-Fajr morning, selected Quiet morning, intended fasting morning, intended Tahajjud-only morning, Ramadan morning, Eid fasting-unavailable morning, or a fallback state.
...
- `relationLinePosition` is `aboveQuickWakeStateSelector` in v1.3.
...
- `wakeBoundaryVisualMode = interactiveEarlyWorshipWindow` only when the day has selected Pre-Fajr, intended fasting, Ramadan Pre-Fajr, or intended Tahajjud-only wake and the active wake time or planned wake anchor is inside the final-third start → Fajr begins window.
...
- The hidden date row is not required in the visible summary in v1.3, but resolved date text may be included in accessibility if the product chooses to preserve date context.
...
- The hidden date row should not receive a visible loading placeholder in v1.3.
...
- Keep the visible date line hidden in v1.3.
...
## 20. Locked initial requirements

These should be treated as v1.3 locked requirements:
...
6. The Gregorian + Hijri date line is hidden temporarily in v1.3.
...
15. The quick wake-state selector is the final base hero row in v1.3; conditional Pre-Fajr intention controls may appear below it when Pre-Fajr is selected.
...
17. The selector must reuse the same liquid-glass surface language as the Next 10 Mornings card and must not use a flat, opaque, or frosted gray visual treatment.
...
20. `Pre-Fajr` sets the target morning to a pre-Fajr wake, defaults to Tahajjud only outside Ramadan, and sets the default wake to 30 min before Fajr begins.
...
23. Quick wake-state selector changes must route through the shared wake-state / alarm-state resolver; the hero renderer must not create, cancel, or schedule alarms directly.
...
45. The early-worship wake adjuster appears only when the resolved day has selected Pre-Fajr, intended fasting, or intended Tahajjud-only and the wake time or planned wake anchor is inside the final-third start → Fajr begins window.
...
- [ ] Fasting and Tahajjud-only days use the early-worship adjuster when final-third start and Fajr begin are available.
...
- [ ] Quick wake-state selector is a single translucent liquid-glass segmented pill using the same surface language as the Next 10 Mornings card.
...
- [ ] `Pre-Fajr` selection sets wake to 30 min before Fajr begins, defaults to Tahajjud only outside Ramadan, and uses early-worship mode when available.
...
- [ ] Relative-label-to-primary-row spacing uses the reduced v0.3/v1.3 gap.
...
## 22. Remaining open ambiguity for next iteration
...
- Whether the Pre-Fajr intention controls should eventually include additional non-fasting reasons beyond Tahajjud only.
...
It should be calmer and simpler than the Weekly Fajrcast card, but just as truthful. In v1.3, the top of the hero prioritizes the location used for the Fajr calculation rather than the date. This makes the resolved prayer-time context immediately visible before the user reads the target morning or wake state.

The v1.3 composition starts with the location line, then the relative day, then the primary wake time or quiet-state text. The compact wake-boundary visual appears below the primary row when eligible. On default Fajr mornings, it shows the Fajr begins → Fajr ends window with endpoint circles at both ends and the alarm icon positioned along the bar. On Pre-Fajr or Tahajjud-only mornings, it shows the final-third start → Fajr begins window with a vertical line/tick at the left boundary, an endpoint circle at Fajr begins, and the alarm icon positioned along the bar. In Quiet mode, it keeps the Fajr begins → Fajr ends visual as static information with no wake marker and no draggable handle.
...
The quick wake-state selector is the final row. It is a single translucent liquid-glass segmented pill with `Pre-Fajr`, `Fajr`, and `Quiet`, using the same surface language as the Next 10 Mornings card rather than a flat, opaque, or frosted gray control. `Fajr` is selected by default. Selecting `Pre-Fajr` moves the target morning into the early-worship wake model, defaults the wake to 30 min before Fajr begins, and defaults the intention to Tahajjud only outside Ramadan. Selecting `Fajr` restores the ordinary Fajr wake model and defaults the wake to 30 min before Fajr ends. Selecting `Quiet` suppresses the wake alarm for the target morning while preserving Fajr begin/end timing context.
