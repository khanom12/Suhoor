...
| Field | Value |
| --- | --- |
| Canonical filename | `subh-weekly-fajrcast-card-spec-v13.md` |
| Version | 13 |
...
| Related specs | `00-subh-spec-index-v1.md`, `subh-morning-resolution-contract-state-ownership-spec-v2.md`, `subh-fajr-time-calculation-determination-selection-spec-v1.md`, `subh-morning-hero-item-spec-v14.md`, `subh-next-10-mornings-wake-forecast-spec-v4.md` |
| Owning domain / surface | Home / Weekly Fajrcast card |
| Implementation audit status | Needs implementation audit |

## Purpose
Define the weekly Fajrcast card as a compact, truthful supporting view of resolved Fajr-window movement and wake context around the anchor morning.

## What This Spec Owns
- Weekly chart geometry, interaction, labels, and accessibility.
- Selected-day and anchor-day behavior.
...
**Weekly Fajrcast is a dark, glass-style seven-day card anchored around the next relevant morning or next alarm, with a fixed Gregorian week pill, top weekday labels, a plotted Fajr window, and a bottom callout that lets the user temporarily inspect the three previous and three next mornings without moving the chart; when touch inspection ends, the card snaps back to the next-alarm focus.**
...
> **The band is the Fajr window. Markers are alarms or planned wake states. The chart window is anchored. Touch temporarily inspects a visible day. Release returns the card to the next alarm.**

This creates three separate concepts:

- **Anchor day:** the center date that defines the fixed seven-day chart window.
- **Resting focus / snap-back target:** the date shown when no chart gesture is active. This is normally the next active alarm or the next relevant morning supplied by the data layer. In the standard card, this is the center anchor day.
- **Inspection focus:** the visible date currently under the user’s finger, tap, drag, scrub, or accessible inspection action.

At rest, the focused day equals the resting focus. During an active touch/scrub, the focused day temporarily becomes the inspection focus. When the touch ends, the guide, emphasized marker, bottom callout, header date pill, and accessibility value return to the resting focus / snap-back target.

The chart does not move, pan, scroll, or recenter during this process. The visible date columns, Fajr band, y-axis scale, top x-axis labels, and static past/elapsed overlay remain anchored while only the temporary focus presentation changes.

The header pill has one intentional exception: at rest it shows the anchored seven-day Gregorian range, but during active touch/scrub it temporarily shows the currently inspected day’s single Gregorian date. When the gesture ends and focus snaps back, the pill returns to the anchored seven-day Gregorian range.

The footer is no longer a focused-day Fajr-time readout. In v13 it is an anchored, week-level trend and context summary. It explains how Fajr begins shifts across the visible week and, only when applicable, surfaces qualifying special non-Ramadan Sunnah fasting opportunities. It must not summarize the weekly alarm plan or repeat the focused-day alarm relation.

## 5. Seven-day window rules

The chart always renders exactly seven day columns:

1. anchor day minus 3
2. anchor day minus 2
3. anchor day minus 1
4. anchor day
5. anchor day plus 1
6. anchor day plus 2
7. anchor day plus 3

The anchor day is always the center column. The card must never silently become a future-only `today + six days` forecast unless today is itself the anchor day.

### Default anchor day and snap-back target

On first load, choose the next relevant morning:

- If today’s relevant wake/alarm moment is still upcoming, anchor on today.
- If today’s relevant wake/alarm moment has passed, anchor on tomorrow.
- If the next relevant morning has no active alarm, it may still be the anchor day; the card must show the correct no-alarm/off/quiet state rather than skipping the date.

The resting focus and snap-back target should normally be the same as the anchor day. If the data layer intentionally supplies a different `snapBackTargetDateKey`, it must still be one of the seven visible dates and must represent the next alarm or next relevant morning. The renderer must not search outside the supplied snapshot to choose a different snap-back date.
...
The visible dates change only when the card receives a new resolved snapshot, such as after a day rollover, a location/settings recalculation, or an explicit external request to rebuild the card around a different anchor day.
...
- the chart remains anchored to the same seven dates
...
- the footer remains the anchored week-level context summary and does not chase the user’s finger
- the screen reader value updates to the touched/focused day and must use correct tense for past, in-progress, and future Fajr windows
...
- the focused guide returns to the snap-back target column, normally the center column
...
- the header pill returns from focused single-date text to the anchored seven-day range text
- the static past/elapsed overlay does not change
- the footer remains the same anchored week-level context summary unless the underlying snapshot changes
...
- in-chart `Fajr begins` and `Fajr ends` labels near the left/past side of the plot
...
The footer must not duplicate the focused-day alarm time, focused-day alarm offset, weekly alarm-plan summary, or exact focused-day Fajr begin/end sentence in v13.
...
- The displayed date must be the currently inspected/focused day, not the anchor day unless the anchor day is also under inspection.
...
The chart always has seven evenly spaced columns. Column 4 is the anchor day and normally the resting next-alarm focus. During active inspection, the focused day may be any visible column; after release, focus returns to the snap-back target.
...
- The overlay is anchored to the resolved next-alarm/resting boundary, not to the user’s finger.
- It must not move, stretch, or re-anchor when the user scrubs across previous or future days.
...
- Both labels are anchored near the left side of the plot, visually associated with the left/past side of the seven-day window.
...
- If a marker overlaps a label, first apply the state-aware side rule: for a pre-Fajr wake pattern, move `Fajr begins` below its boundary; otherwise keep the default above/below placement.
- If overlap remains, nudge the label within a small leading label lane while keeping it near the left/past side; then recompute the boundary angle if the horizontal anchor changed.
...
The chart band, boundary strokes, in-chart labels, accessibility summary, and detail payload must use the same Fajr begin/end values. The v13 footer is a week-level trend/context summary and is not the authoritative location for exact focused-day Fajr begin/end times.
...
The y-axis scale is calculated from the current anchored seven-day window.
...
- callout: wake time
- footer: anchored week-level context summary only; no redundant visible alarm-relation line
...
- callout: `Off`
- footer: anchored week-level context summary only
...
- visible footer remains the anchored week-level context summary
...
At rest, the callout aligns with the snap-back target, normally the center next-alarm/anchor column. During active scrub or press, it temporarily aligns with the inspected column. On release, it returns to the snap-back target with the guide, marker emphasis, header pill, and accessibility value.
...
The v13 footer is a compact **week-level Fajr trend and context summary**, not a focused-day Fajr-time readout and not a weekly alarm-plan summary.
...
At rest and during chart scrubbing, the footer describes the anchored visible week. It does not chase the user’s finger. When the user releases a scrub, the footer normally remains unchanged unless the entire snapshot changes.
...
- **Line 1:** weekly Fajr trend line for the anchored seven-day window.
...
- Use the first visible day and last visible day in the anchored seven-day window to describe the weekly change.
...
Line 2 is optional. It should appear only when the data layer supplies a qualifying **special non-Ramadan Sunnah fasting** context for the anchored visible week.
...
### Forbidden visible footer patterns in v13
...
The compact card header date pill is Gregorian-only in v13.
...
- Past visible days do not require completion-history data to appear in the chart. If no history record exists for a previous day, still show the date using calendar-derived Gregorian values.
...
The compact card may support several contextual day meanings, but it should not become chip-heavy or visually crowded. In v13, special context primarily affects:
...
- preserve the seven-day rhythm
- cap maximum width through the containing layout if needed
- keep the in-chart Fajr labels near the left/past side instead of letting them drift toward the center of a very wide chart
...
`weekPillText` is the resting range-mode pill text for the anchored seven-day Gregorian window. During active inspection, the renderer should display the focused day’s Gregorian `datePillText` in the same pill instead. On release, the renderer must return the pill display to `weekPillText`.
...
`weekFooterPrimaryText` is the anchored weekly Fajr trend line. `weekFooterSecondaryText`, when present, is the qualifying special non-Ramadan Sunnah fasting opportunity line. These are not focused-day strings and should not change merely because the user scrubs within the seven visible days.

The snap-back target must be supplied by the data layer. It is normally the same as `anchorDateKey` and represents the next alarm or next relevant morning. The renderer must not infer a different target by scanning for the nearest marker.
...
- Alarm-plan fields such as default offsets, quiet days, no-alarm weeks, adjusted mornings, and off states may exist elsewhere in the app, but they are not footer content in this v13 card.
...
- `visibleDays` always equals anchor day minus 3 through anchor day plus 3.
...
- `weekFooterPrimaryText`, optional `weekFooterSecondaryText`, and `accessibilitySummary` must match the anchored week and current focused-day accessibility state.
...
- `weekPillText` must be preformatted for resting mode using the anchored seven-day Gregorian range only, for example `April 26–May 2`.
...
3. The anchor day is always the center column.
4. The visible window is anchor day minus 3 through anchor day plus 3.
...
13. The header pill describes the anchored visible Gregorian window at rest, temporarily shows the inspected day’s single Gregorian date during active scrub, and returns to the anchored range on release.
...
20. Footer line 1 summarizes how Fajr begins changes across the anchored visible week.
...
29. The in-chart Fajr labels sit near the left/past side of the plot and do not move with scrubbing.
...
36. No-alarm/no-anchor days have no dot, circle, icon, or placeholder marker.
...
39. The static past/elapsed overlay is anchored to the next-alarm/resting boundary and does not move with scrubbing.
...
- [ ] Anchor day is centered.
- [ ] Window is anchor day D-3 through D+3.
- [ ] Initial anchor follows next-relevant-morning logic.
- [ ] Snap-back target is supplied and normally equals the centered next alarm / next relevant morning.
...
- [ ] Active scrub does not change the anchored week-level footer.
- [ ] Past scrubbed days use past-tense accessibility wording.
...
- [ ] Header pill reflects the anchored seven-day Gregorian range at rest.
- [ ] Header pill switches to the inspected day’s single Gregorian date during active scrub.
- [ ] Header pill returns to the anchored seven-day range on release.
...
- [ ] Fajr labels are near the left/past side of the plot.
...
- [ ] Fajr label normal offsets place `Fajr begins` above its line by default, but below its line for pre-Fajr wake patterns.
...
- [ ] Line 1 explains how Fajr begins changes across the anchored visible week.
...
The Weekly Fajrcast card should feel like a premium morning forecast for Fajr-centered life: minimal, readable, calm, precise, anchored, and interaction-aware.

At a glance, the user should understand the visible week’s Fajr window and the next-alarm/resting morning. With one touch, they should inspect the previous three or next three mornings without the chart moving away. While they scrub, the header pill should show the inspected day’s Gregorian date only, and the bottom callout should show the inspected day’s alarm or wake state. When they release, the card should calmly return to the next alarm and the pill should return to the full Gregorian week range. The bottom callout should feel precisely centered in its chart-to-footer pocket, and the `Fajr begins` / `Fajr ends` labels should read as native annotations attached near the sloped boundary lines, angled with them, but never printed directly on top of them.

The v13 visual direction keeps the secondary-calendar text removed from the pill, keeps weekday labels at the top of the plot, keeps the focused-day callout below the plot, and keeps readable in-chart `Fajr begins` and `Fajr ends` labels near the left/past side of the chart. The footer becomes a concise week-level Fajr trend/context summary: line 1 explains how Fajr begins changes across the week, and line 2 appears only for qualifying special non-Ramadan Sunnah fasting opportunities.
