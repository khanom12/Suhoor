# Next 7 Days Wake Forecast Specification

| Field | Value |
| --- | --- |
| Canonical filename | `subh-next-7-days-wake-forecast-spec-v1.md` |
| Version | 1 |
| Spec status | Product / implementation direction; canonical Desktop working spec |
| Supersedes | `subh-next-10-mornings-wake-forecast-spec-v4.md` |
| Related specs | `00-subh-spec-index-v2.md`, `subh-morning-resolution-contract-state-ownership-spec-v3.md`, `subh-quick-wake-mode-intent-mutation-contract-v2.md`, `subh-planning-horizon-day-resolution-intention-anchoring-spec-v3.md`, `subh-alarm-detail-view-screen-spec-v7.md` |
| Owning domain / surface | Home / Next 7 Days forecast surface |
| Implementation audit status | Needs implementation audit |

## Purpose
Define the compact and expanded forecast that previews the next seven resolved Fajr-centered days without becoming a second wake engine.
...
## Next 7 / Weekly Fajrcast Alignment Addendum

This addendum is normative for v1 and supersedes any lower prose inherited from the archived Next 10 spec that implies ten rows, a separate forecast date set, or a forecast starting after the immediate target morning.

- The visible Home forecast is now `Next 7 Days`, not `Next 10 Mornings`.
- The expanded forecast shows exactly seven rows in the ready state.
- Row 1 is the next immediate alarm or next relevant morning supplied by the canonical resolver.
- Rows 2–7 are the six following calendar mornings.
- The seven `visibleDateKeys` must match the Weekly Fajrcast `visibleDateKeys` exactly and in the same order.
- Forecast visibility does not make every visible day operationally scheduled.
- Forecast rows still consume resolved morning/day snapshots and must not infer intention, Fajr boundaries, delivery status, or opportunity tags locally.

## 1. Scope

This specification defines the **Next 7 Days Wake Forecast** card as a standalone product, UX, visual, data, tagging, interaction, collapsed/expanded, and accessibility component.

It replaces the archived “10-Day Wake Forecast / Next 10 mornings” presentation with a cleaner, single-header, tag-driven forecast list.

This revision also locks the Home behavior:

```text
Next 7 Days is collapsed by default on Home.
The header remains visible.
The user can expand it to reveal the seven-row list.
The seven rows are the same visible dates, in the same order, as Weekly Fajrcast.
```

The component must preserve the current premium glass visual direction while simplifying the information architecture of each row and aligning its visible date set with the Weekly Fajrcast card.
...
Weekly Fajrcast is aligned to the same seven visible dates but remains a separate chart spec and is not collapsed by this specification.

## 2. One-sentence definition

**Next 7 Days is a dark, glass-style wake forecast that appears collapsed by default on Home with its header visible, and expands into a seven-row list showing only three visible row concepts: the date, compact state tags, and the resolved wake time/status.**
...
Do not show archived or generic two-line titles such as:
...
as a two-line header. The visible header remains the single line `NEXT 7 DAYS`.
...
```swift
NextSevenDaysCard
NextSevenDaysRowView
NextSevenDaysPresentation
NextSevenDaysTagResolver
...
2. **Expanded state** — header visible, seven forecast rows visible.
...
- Do not show the seven rows while collapsed.
...
- do not remove row separation entirely; seven large time rows need structure

Divider placement:

- one divider after the header
- one divider between each row
- optional final divider after row 7 if visually needed
...
All seven rows must use the same `dateLaneWidth`, `tagLaneWidth`, `timeLaneWidth`, `middleLaneLeadingX`, `middleLaneTrailingX`, and `tagLaneCenterX` for a given resolved snapshot and text-size environment.
...
- The tag cluster horizontal centerline must align across all seven rows.
...
The exact implementation may use `Grid`, a measured layout helper, a custom `Layout`, or a `PreferenceKey` measurement pass. The requirement is the alignment result: **tags are perfectly centered in the same middle lane across all seven rows.**
...
```swift
struct NextSevenDaysSnapshot: Equatable {
    let title: String                 // "NEXT 7 DAYS"
    let rows: [NextSevenDaysRowDisplay] // exactly 7 when ready
...
```swift
struct NextSevenDaysRowDisplay: Equatable, Identifiable {
    let id: String                    // dateKey
    let dateKey: String
    let date: Date
    let dateLabel: String             // Today, Tomorrow, Sat, May 2
    let tags: [NextSevenDaysTagDisplay]
...
```swift
struct NextSevenDaysTagDisplay: Equatable, Identifiable {
    let id: String
    let title: String                 // Fajr, Fasting, Ashura, etc.
    let semantic: NextSevenDaysTagSemantic
    let prominence: NextSevenDaysTagProminence
    let priority: Int
...
```swift
enum NextSevenDaysTagSemantic: Equatable {
    case fajrFallback
    case quietMode
    case ramadan
    case fastingIntent
...
```swift
enum NextSevenDaysTagProminence: Equatable {
    case strong
    case opportunity
    case fallback
}
...
```swift
struct NextSevenDaysTagResolverInput {
    let date: Date
    let dateKey: String
...
When expanded and ready, the card shows exactly seven upcoming Fajr-centered days.

Default behavior:

- first row: the next immediate alarm or next relevant morning supplied by the canonical data layer;
- if today's relevant wake/alarm moment is still upcoming, the first row may be today;
- if today's relevant wake/alarm moment has passed, the first row is normally tomorrow;
- rows 2–7: the following six calendar mornings.

The Home hero remains the immediate answer surface, but the forecast now includes that same target morning as row 1 so the Home forecast and Weekly Fajrcast describe the same upcoming week.

Weekly Fajrcast alignment:

- `visibleDateKeys` for Next 7 Days must equal `visibleDateKeys` for Weekly Fajrcast.
- The order must be identical: forecast start day, then the following six days.
- Neither surface should include previous mornings in this aligned MVP behavior.
- Neither surface should silently skip a quiet/no-alarm day merely to find an active marker.
...
- The `NEXT 7 DAYS` header remains visible.
- The rows are hidden.
...
```text
WakeRowEntry
  -> NextSevenDaysPresentation.row(...)
  -> NextSevenDaysRowDisplay
  -> NextSevenDaysRowView
```
...
- collapsed Home state shows the `NEXT 7 DAYS` header
- collapsed Home state hides the seven rows
- expanding the card reveals exactly seven rows when ready
- collapsing the card hides rows without mutating wake state
- header title is `NEXT 7 DAYS`
- old title `7-Day Wake Forecast` is not visible
- subtitle `Next 7 Days` is not visible as separate text
...
1. Visible header is `NEXT 7 DAYS`.
2. Do not show `7-DAY WAKE FORECAST`.
3. Do not show a second header subtitle.
4. The card is collapsed by default on Home with the `NEXT 7 DAYS` header visible.
5. The expanded card shows seven upcoming Fajr-centered days in the ready state.
...
12. Tags are centered in a shared middle lane that aligns across all seven rows.
...
- [ ] First row uses the resolved forecast-start label: `Today` when the relevant wake/alarm moment is still upcoming, otherwise normally `Tomorrow`.
...
The Next 7 Days card should feel like a calm, premium wake forecast for Fajr-centered life.
...
This gives the user the essence of the next seven days without clutter: date, meaning, wake time.
