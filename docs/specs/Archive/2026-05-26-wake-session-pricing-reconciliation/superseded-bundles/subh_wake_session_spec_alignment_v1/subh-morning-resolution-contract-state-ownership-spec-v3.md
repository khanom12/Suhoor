...
| Version | 3 |
| Spec status | Draft; reconciled Desktop working spec; aligned to Next 7 Days / Weekly Fajrcast horizon |
...
This addendum is normative for v3 and supersedes lower references to `Next 10` as the active Home forecast surface.

- Surface snapshots must support `Next 7 Days`, not `Next 10 Mornings`, as the active near-term forecast surface.
- `Next 7 Days` and Weekly Fajrcast consume the same seven resolved date keys in the same order.
...
For any morning date, Subh must produce one canonical resolved morning object graph. Home Hero, Alarm Detailed View, Weekly Fajrcast, Next 7 Days, alarm scheduling, notification copy, completion logic, and analytics must consume that resolved graph instead of independently re-deriving prayer windows, wake modes, day meaning, or alarm state.
...
3. Next 7 Days rows infer `[Fasting]` from tags even though the user has not intended a fast.
...
| Next 7 Days v1 | seven-row visual/tag contract | rule that rows consume resolved tags and must not infer intention locally |
...
8. **Next 7 Days** — seven-row forecast layout, tags, date alignment with Weekly Fajrcast, and accessibility.
...
Next 7 Days row → infers fasting intention from tag text
...
- `visibleDateKeys` powers surfaces such as Next 7 Days and month browsing.
...
| Fajr ends | prayer-time resolver | Hero, Fajrcast, Next 7 Days, scheduler | renderer-invented offset |
...
AND Next 7 Days may show [Fajr] [White Days]
```
...
### 20.21 Next 7 Days tag doctrine

```gherkin
GIVEN an opportunity-only Ashura morning
WHEN Next 7 Days row tags are resolved
...
```gherkin
GIVEN Next 7 Days and a month view display dates outside the active scheduled horizon
...
- Next 7 Days snapshot tag rules
...
- Next 7 Days row tags without explanatory subtitles
...
2. Home Hero, Alarm Detail, Weekly Fajrcast, and Next 7 Days consume resolved snapshots rather than duplicating domain logic.
