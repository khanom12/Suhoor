...
| Version | 3 |
| Spec status | Draft; reconciled Desktop working spec; aligned to Next 7 Days horizon |
...
This addendum is normative for delivery and supersedes lower references to scheduling `Next 10` rows.

- Delivery must not schedule events merely because they appear in `Next 7 Days` or Weekly Fajrcast.
- `Next 7 Days` and Weekly Fajrcast may show the same seven visible dates, but that display horizon is still distinct from the active scheduled horizon.
...
The delivery layer consumes resolver-materialized events that the parent morning/window builder marks as operational. It must not schedule events merely because they appear in Next 7 Days, Weekly Fajrcast, month browsing, or a day detail view.
...
- Next 7 Days row layout or tag doctrine
...
| Next 7 Days | compact row forecast and tags | consumes schedule/delivery status only as resolved row state; no delivery diagnostics |
...
- The delivery layer must not expand scope by reading calendar browsing data or Next 7 Days visible rows.
...
```text
Schedule all Next 7 Days rows because Next 7 Days is visible.
...
Next 7 Days may use concise status if a wake time is unavailable or quiet.
...
- GIVEN Next 7 Days shows seven resolved days
