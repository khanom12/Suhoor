...
| Supersedes | `subh-pricing-entitlement-spec-v2.md` |
...
`Complete Lifetime` is removed from the active MVP pricing model.

`Founder Complete`, `Complete Annual`, `Complete Monthly`, and `Complete Lifetime` product concepts are removed from the active MVP pricing model.
...
| `Complete Lifetime` | Removed from MVP pricing. Lifetime may be reconsidered later only through a dedicated pricing decision. |
...
- Next 7 Days forecast display;
- Next 7 Days editing for supported wake-plan controls;
...
| Next 7 Days forecast display | Yes | Yes |
| Next 7 Days editing | Yes | Yes |
...
- Entitlement must not treat Weekly Fajrcast as a different date horizon from Next 7 Days.
...
- [ ] No Complete Lifetime tier exists in MVP pricing.
...
- [ ] Free can use Next 7 Days and Weekly Fajrcast.
...
- [ ] Tapping Fajr, Suhoor, Quiet, wake adjustment, Wake Checks, awake confirmation, active/current-morning check-in, Next 7 Days, or Weekly Fajrcast does not open a paywall.
...
- Weekly Fajrcast;
- Next 7 Days forecast display and editing;
...
- create or preserve a Complete Lifetime tier;
...
- gate Next 7 Days behind payment;
