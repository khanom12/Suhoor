...
| Version | 15 |
| Spec status | Product / implementation direction; canonical Desktop working spec; aligned to Next 7 Days horizon |
| Supersedes | `subh-morning-hero-item-spec-v14.md`; Archive/Morning_Hero_Item_Specification_v13.md |
| Related specs | `00-subh-spec-index-v3.md`, `subh-morning-resolution-contract-state-ownership-spec-v3.md`, `subh-quick-wake-mode-intent-mutation-contract-v2.md`, `subh-early-worship-boundary-spec-v2.md`, `subh-next-7-days-wake-forecast-spec-v1.md`, `subh-weekly-fajrcast-card-spec-v14.md`, `subh-wake-sessions-wake-checks-morning-logs-spec-v1.md`, `subh-quiet-mode-quiet-morning-contract-spec-v1.md`, `subh-sound-alarm-settings-spec-v1.md` |
...
## v15 Next 7 Days / Weekly Fajrcast Alignment Addendum

This addendum is normative for v15 and supersedes lower Home-surface references to `Next 10 Mornings`.

- On Home, the near-term forecast card is `Next 7 Days`.
- `Next 7 Days` appears collapsed by default with its `NEXT 7 DAYS` header visible.
- When expanded, it shows seven rows: the next immediate alarm / next relevant morning plus the following six mornings.
- Weekly Fajrcast uses the same seven visible dates as Next 7 Days.
...
The earlier implementation visually showed a Home hero selector with `Fast | Fajr | Quiet`, while the detail screen used `Early | Fajr | Quiet`. v15 preserves the prior alignment of both surfaces to the same mode labels:
...
26. The `Pre-Fajr | Fajr | Quiet` selector must use the same translucent liquid-glass surface language as the Next 7 Days card, not an opaque or frosted gray segmented-control treatment.
...
32. On Home, `Next 7 Days` appears collapsed by default with its header visible.
33. The Weekly Fajrcast remains a separate card, is not collapsed by this revision, and uses the same seven visible dates as Next 7 Days.
...
Visual aesthetic rules remain unchanged from v12: the selector must match the translucent, premium liquid-glass language used by the Weekly Fajrcast card and the Next 7 Days card, and it must not use a separate opaque gray segmented-control style.
...
- `Next 7 Days` appears collapsed by default below the hero.
- The `NEXT 7 DAYS` header remains visible in the collapsed state.
...
- The selector must feel glassy and integrated with the background, matching the Next 7 Days card surface language rather than an opaque or frosted gray segmented-control treatment.
...
| Quick wake-state selector pill | visual token | same liquid-glass surface family as the Next 7 Days card; three equal logical segments; selected highlight moves smoothly; 44 px/pt effective hit target per segment |
...
17. The selector must reuse the same liquid-glass surface language as the Next 7 Days card and must not use a flat, opaque, or frosted gray visual treatment.
...
- [ ] Quick wake-state selector is a single translucent liquid-glass segmented pill using the same surface language as the Next 7 Days card.
