# Subh Wake Session Testing, Simulation, and Real Alarm Validation Harness Spec

| Field | Value |
| --- | --- |
| Canonical filename | `subh-wake-session-testing-and-simulation-harness-spec-v4.md` |
| Version | 4 |
| Spec status | Draft canonical candidate; May 30, 2026 Quiet / Pause / Hero / Wake Flow testing realignment |
| Date | 2026-05-30 |
...
3. “Quiet during active follow-up alarms” is removed from the active testing model. Once the first alarm begins, Quiet is unavailable.
...
Fasting today status/intention
Fajr prayer logged
No response
...
9. Quiet is not exposed after the first alarm begins.
...
13. `I’m awake`, `I’m fasting today`, and `I prayed Fajr` are separate actions/log facts.
...
No Quiet/Pause options may appear in this selector.
...
- `I’m awake` / `I’m awake for Fajr` / `I’m fasting today` / `I prayed Fajr` actions;
...
| --- | --- | --- | --- | --- |
| Active Fajr planning | Fajr alarm time | Enabled Fajr timeline | Wake time relative to Fajr end | `[ Fajr | Suhoor ]` |
| Active Suhoor planning | Suhoor alarm time | Enabled Suhoor timeline | Wake time relative to Fajr begins | `[ Fajr | Suhoor ]` |
| Quiet Fajr | `Quiet` | Saved Fajr time visible but inactive/quieted | Saved alarm time + will not ring this morning | `[ Fajr | Suhoor ]` |
| Quiet Suhoor | `Quiet` | Saved Suhoor time visible but inactive/quieted | Saved alarm time + will not ring this morning | `[ Fajr | Suhoor ]` |
| Global Pause | `Alarms paused` | Saved purpose time visible but paused/ghosted | Wake alarms stay off until resumed | `[ Fajr | Suhoor ]` |
| Ring this/tomorrow only | `Rings this morning only` or `Rings tomorrow only` | Active one-morning timeline | Global pause remains after this morning | `[ Fajr | Suhoor ]` |
| Alarm setup issue | Clear issue label | Timeline disabled or repair state | Repair guidance | Repair CTA, not Quiet |
| Primary alarm active | `Time to wake` | Active wake-session state | Tap when you are awake | `[ I’m awake ]` |
| Follow-up pending | `Next alarm soon` | Follow-up countdown/sequence state | Next alarm time/countdown | `[ I’m awake ]` |
| System dismissal acknowledged | `You’re awake` | Remaining alarms cancelled | Confirmed from system alarm | checked awake state / next valid CTA |
| In-app awake acknowledged | `You’re awake` | Remaining alarms cancelled | Confirmed in Subh | checked awake state / next valid CTA |
| Suhoor acknowledged | `You’re awake for Suhoor` | Suhoor session complete | Fajr begins at calculated time | `[ I’m fasting today ]`, when eligible |
| Fasting today logged | fasting status visible where specified | no alarm mutation | Fasting today recorded as test-only | checked fasting state or next valid CTA |
...
| Fajr wake confirmed | `You’re awake for Fajr` | Fajr window timeline | Prayer confirmation available/soon | `[ I prayed Fajr ]` when eligible |
...
```text
Wake purpose selector: Fajr | Suhoor
...
| W2 | Fajr follow-up pending | Follow-up UI | Next alarm time visible; `I’m awake` cancels remaining |
| W3 | Fajr in-app acknowledgement | In-app wake confirmation | acknowledgedBy = inAppButton; checks cancelled |
| W4 | Fajr system dismissal acknowledgement | System wake confirmation | acknowledgedBy = systemAlarmDismiss; checks cancelled where supported |
...
| S2 | Suhoor awake → I’m fasting today | Fasting status | Fasting today logged separately; no fast completion |
...
For the current MVP model, explicit system / AlarmKit dismissal must be tested as an awake acknowledgement.
...
→ remaining follow-up alarms cancelled
→ Home updates to acknowledged state
→ test log records system acknowledgement
```
...
System dismissal acknowledgement cancels remaining follow-ups where supported
...
system dismissal acknowledgement records source
...
Fasting today does not log fast completion
...
Primary alarm active
Follow-up pending
System dismissal acknowledged
In-app awake acknowledged
Suhoor awake
Fasting today logged
...
Suhoor wake → fasting today → Fajr wake → Fajr prayer
System dismissal → acknowledgement source → follow-ups cancelled
...
8. Confirm Slot 6 shows only `[ Fajr | Suhoor ]` before execution.
...
6. Tap or step to `I’m fasting today`.
...
| System dismissal cancels remaining follow-ups | Yes | Current MVP model |
...
- [ ] Quiet is unavailable after first alarm begins.
- [ ] Active alarm states show `I’m awake`, not `Stop checks`.
- [ ] System dismissal is tested as awake acknowledgement for MVP, with source captured where supported.
- [ ] Suhoor wake acknowledgement and Fajr wake acknowledgement are separate.
- [ ] Fasting today and Fajr prayer logging are separate.
