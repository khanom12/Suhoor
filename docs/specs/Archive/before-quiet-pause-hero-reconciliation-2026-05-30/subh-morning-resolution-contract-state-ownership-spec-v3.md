...
- The Wake Session owns execution state for one operational morning: primary alarm, wake checks, awake confirmation, quiet cancellation, expiry, and reconciliation.
...
- Selecting Suhoor sets `fastingDayPlanned` for the resolved morning. Tapping `I’m awake for Suhoor` confirms Suhoor wake and sets `fastingIntentConfirmed` for that morning.
- Tapping `I’m awake for Fajr` confirms Fajr awake status only. It must not mark Fajr prayer as prayed.
...
For any morning date, Subh must produce one canonical resolved morning object graph. Home Hero, Alarm Detailed View, Weekly Fajrcast, Next 7 Mornings, alarm scheduling, notification copy, completion logic, and analytics must consume that resolved graph instead of independently re-deriving prayer windows, wake modes, day meaning, or alarm state.
...
6. Historical selector-label conflict is resolved for MVP: Home and Detail expose `Suhoor | Fajr | Quiet`; Fast/Early/Tahajjud-only wording is legacy or deferred unless a future spec reintroduces it.
