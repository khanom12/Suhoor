...
## May 29 Testing Harness Scenario Alignment Addendum

The testing harness must include task-oriented scenarios for the new Quiet/Pause/Hero state model, without overloading the tester with subsystem names.

Required scenario groups:

```text
Planning: Active Fajr, Active Suhoor
Silent: Quiet Fajr, Quiet Suhoor
Paused: Paused Fajr, Paused Suhoor
Exception: Rings tomorrow only while paused
Setup/issue: Turn on alarms, Set location, Alarm issue
Execution: Time to wake, Next alarm soon, Final alarm this morning
Post-awake Fajr: I’m awake → I prayed Fajr → Fajr complete
Post-awake Suhoor: I’m awake → I’m fasting today → Fajr has begun → I prayed Fajr
Boundary: no follow-up alarms when wake is too close to Fajr begins/end
Handoff: Fajr end switches hero to next morning
```

The harness should expose scenario cards using user-facing labels first, with developer diagnostics secondary.
...
This spec defines a comprehensive internal testing layer that lets Omar and internal testers personally verify Subh's Wake Sessions, Follow-up Alarms, Home Hero states, Fajr/Suhoor/Quiet interactions, MorningLogs, and real AlarmKit behavior without waiting for real mornings.
...
2. **Run Real Alarm Test** — map a simulated wake session onto real near-future AlarmKit alarms while preserving five-minute follow-up-alarm spacing.
...
```text
Follow-up alarm interval = 5 minutes
Maximum follow-up alarms after primary = 5
Maximum total attempts = primary + 5 follow-up alarms
```

State jumps may be instant. Real mapped alarm playback preserves production follow-up-alarm spacing.

This means:

- Preview Home UI can instantly jump to `follow-up alarm 3 pending` without waiting.
- Real Alarm Test schedules follow-up alarms five real minutes apart.
- No v3 scenario should use one-minute or two-minute follow-up-alarm spacing.
...
3. Quiet mode.
4. Wake Sessions.
5. Follow-up Alarms with five-minute spacing.
6. Alarm stop vs awake confirmation.
7. `I’m awake`.
8. `I’m awake`.
9. `I prayed Fajr`.
10. Fasting intent after Suhoor.
11. Quiet during active follow-up alarms.
...
- adaptive follow-up-alarm behavior;
- public advanced follow-up-alarm settings;
- changing the iPhone system clock;
- changing production follow-up-alarm spacing.
...
What this tests:
The awake confirmation, follow-up-alarm pending state, Hero Action Slot, and Fajr prayer confirmation.
...
What this tests:
Real AlarmKit ringing, Stop, Open Subh, and five-minute follow-up alarms.
...
What to expect:
The primary alarm rings first. If you stop it without confirming awake, the next follow-up alarm rings five minutes later.
...
1. `Fajr Flow`
2. `Suhoor Flow`
3. `Quiet During Follow-up Alarms`
4. `Custom Date & Time`
...
### 11.5 Quiet During Follow-up Alarms card

Purpose:

```text
Preview what happens when Quiet is selected while follow-up alarms are active.
```
...
- follow-up-alarm pending states;
...
Simulated event: {primary/follow-up alarm/etc.}
Real fire time: {time}
```
...
```text
Expected: The Hero should show “I’m awake” only during the active alarm/follow-up alarm flow.
Expected: The Hero should show “I’m awake” only during the active alarm/follow-up alarm flow.
Expected: Tapping Quiet should show the stop follow-up alarms sheet.
...
- Suhoor window open;
- primary Suhoor alarm fired;
- Suhoor follow-up alarms pending;
- awake for Suhoor confirmed;
- fasting intent confirmed;
- Suhoor confirmed → Fajr begins → `I prayed Fajr`;
- Suhoor unacknowledged → Fajr begins → `I prayed Fajr` when eligible;
- Quiet active;
- Quiet during pending follow-up alarms confirmation sheet;
- quietMorning logged;
...
Default: primary + 5 follow-up alarms.
...
Simulated follow-up alarm 1: 4:35 AM
Simulated follow-up alarm 2: 4:40 AM

Real now: 2:00 PM
Mapped primary wake: 2:02 PM
Mapped follow-up alarm 1: 2:07 PM
Mapped follow-up alarm 2: 2:12 PM
```
...
Mapped playback must preserve the production follow-up-alarm interval:
...
```text
1-minute follow-up alarms
2-minute follow-up alarms
compressed follow-up-alarm spacing
```
...
preserving follow-up-alarm deltas after the mapped primary alarm
```
...
```text
No follow-up alarm later than 5 minutes before Fajr ends.
```

Suhoor mapped playback:

```text
No follow-up alarm later than 5 minutes before Fajr begins.
```
...
- pending mapped follow-up alarms remain scheduled unless cancelled by confirmation, Quiet, or Exit Test Mode.
...
- `I’m awake` is separate from `I prayed Fajr`;
...
1. User does not confirm `I’m awake`.
2. Suhoor wake expires unconfirmed.
3. Simulated Fajr begins.
4. Home does not show a second `I’m awake` prompt at Fajr begins.
...
### 20.4 Scenario D — Quiet During Active Follow-up Alarms
...
3. If user taps `Keep follow-up alarms`, no cancellation occurs.
...
5. Old follow-up-alarm IDs cancel.
6. New primary alarm schedules.
7. New follow-up alarms schedule within cutoff.
...
4. Follow-up alarms remain pending.
5. Next follow-up alarm fires if it was scheduled.
6. User opens Subh and taps `I’m awake`.
...
Real follow-up alarm 1: primary + 5 min
Real follow-up alarm 2: primary + 10 min
Real follow-up alarm 3: primary + 15 min
Real follow-up alarm 4: primary + 20 min
Real follow-up alarm 5: primary + 25 min
```
...
5. Silent mode on.
6. Focus mode on.
7. Stop primary and verify follow-up alarm still fires.
...
- follow-up alarms are cancelled after confirmation;
- follow-up alarms remain five minutes apart.
...
4. Confirm Home opens and shows `TEST MODE ACTIVE`.
5. Tap `I’m awake` when available.
...
1. Start `Quiet During Follow-up Alarms` preview.
2. Confirm Home shows follow-up alarms pending.
3. Select Quiet.
4. Confirm the stop follow-up alarms sheet appears.
5. Tap `Keep follow-up alarms` once.
6. Confirm follow-up alarms remain pending.
...
4. Choose sequence length, default `Primary + 5 follow-up alarms`.
...
12. Wait five minutes for follow-up alarm 1.
13. Open Subh.
14. Tap `I’m awake`.
15. Confirm remaining follow-up alarms cancel.
...
8. Tap `I’m awake`.
9. Confirm remaining mapped follow-up alarms cancel.
...
- Fajr follow-up-alarm schedule math with five-minute intervals;
- Suhoor follow-up-alarm schedule math with five-minute intervals;
- cutoff behavior;
- no follow-up alarm after cutoff;
...
- sequence selector: primary only through primary + 5 follow-up alarms;
- mapped playback preserving five-minute follow-up-alarm deltas;
...
| Follow-up alarm after primary stop | Yes | Core Wake Session behavior. |
| Five-minute follow-up-alarm spacing | Yes | Must be preserved in Real Alarm Test. |
...
- production follow-up alarms remain five minutes apart;
...
- [ ] Omar can inspect Fajr, Suhoor, Quiet, follow-up-alarm, confirmation, prayer, and permission states without waiting for real mornings.
- [ ] Omar can jump between named states instantly from Home.
- [ ] Omar can run Real Alarm Test on his iPhone.
- [ ] Omar can choose sequence length from primary only through primary + 5 follow-up alarms.
- [ ] The default mapped sequence is primary + 5 follow-up alarms.
- [ ] Real mapped follow-up alarms are five minutes apart.
...
Do not implement paid features, StoreKit, analytics, adaptive follow-up alarms, household features, or cloud sync.
...
- Do not use one-minute or two-minute follow-up-alarm intervals.
- State jumps may be instant.
- Real Alarm Test must preserve the production five-minute follow-up-alarm interval.
- The Real Alarm Test builder must let the tester choose primary only, primary + 1, primary + 2, primary + 3, primary + 4, or primary + 5 follow-up alarms.
- Default Real Alarm Test sequence is primary + 5 follow-up alarms.
...
10. Real Alarm Test mapping:
   ...
   - preserve five-minute follow-up-alarm spacing;
...
- Confirming awake must cancel remaining follow-up alarms.
...
| Should real AlarmKit mapped tests preserve five-minute follow-up alarms? | Yes. Locked in v2/v3. |
| Should Preview Home UI allow instant jumps? | Yes. Locked in v3. |
| Should mapped playback default to full sequence? | Yes: primary + 5 follow-up alarms. |
...
Five-minute follow-up alarms preserved.
Clear test labels.
