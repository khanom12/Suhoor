# Subh Quiet, Pause, Hero, and Morning Wake Flow Alignment Specification v1

| Field | Value |
| --- | --- |
| Canonical filename | `subh-quiet-pause-hero-wake-flow-alignment-spec-v1.md` |
| Version | 1 |
| Spec status | Canonical cross-spec alignment addendum for the May 29 Quiet/Pause/Hero pass |
| Date | 2026-05-29 |
...
| Related specs | `00-subh-spec-index-v3.md`, Morning Resolution, Planning Horizon, Quick Wake Mutation, Quiet Morning, Wake Sessions, Alarm Delivery, Morning Hero, Alarm Detail, Next 7 Mornings, Month Planning, Pricing/Tier Matrix, Testing Harness |
...
- Slot 6 uses the same physical row for `Fajr | Suhoor`, `I’m awake`, `I’m fasting today`, `I prayed Fajr`, and checked status pills.
...
→ hero shows You’re awake
→ after at least 60 seconds and before Fajr begins, show I’m fasting today
→ user taps I’m fasting today
→ hero shows Fasting today / ✓ I’m fasting today
→ at Fajr begins, the Fajr event still occurs
→ the hero does not show I’m awake again at Fajr begins
→ after the Fajr prayer CTA is eligible, show I prayed Fajr
→ at Fajr end, hero switches to the next morning
```

`I’m fasting today` records fasting intention/status for the day. It does not record fast completion.

If the user does not tap `I’m fasting today` before Fajr begins, the hero should prioritize the Fajr prayer CTA after Fajr begins. Fasting can be logged through an appropriate log/detail surface later if supported.
...
```text
visible when:
  user acknowledged the alarm, AND
