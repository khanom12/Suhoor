...
| Field | Value |
| --- | --- |
| Canonical filename | `subh-alarm-delivery-schedule-reliability-spec-v3.md` |
| Version | 3 |
| Spec status | Draft; reconciled Desktop working spec; aligned to Next 7 Mornings horizon |
| Supersedes | None recorded in the active Desktop set |
| Related specs | `00-subh-spec-index-v3.md`, `subh-morning-resolution-contract-state-ownership-spec-v3.md`, `subh-planning-horizon-day-resolution-intention-anchoring-spec-v3.md`, `subh-quick-wake-mode-intent-mutation-contract-v2.md`, `subh-fajr-time-calculation-determination-selection-spec-v1.md`, `subh-wake-sessions-wake-checks-morning-logs-spec-v1.md`, `subh-quiet-mode-quiet-morning-contract-spec-v1.md`, `subh-sound-alarm-settings-spec-v1.md` |
...
## May 29 Delivery Status / Pause / Wake Flow Alignment Addendum
...
Alarm delivery must distinguish Quiet, inherited Pause, ring-once exception, setup/permission block, and delivery issue.

Required schedule/delivery statuses include at minimum:

```text
scheduled
notScheduledBecauseQuiet
notScheduledBecausePaused
scheduledDespitePause
notScheduledBecauseSetupNeeded
notScheduledBecausePermissionDenied
platformDeliveryIssue
staleDeliveryCancelled
```

Do not map `offWithAnchor`, permission failures, or delivery failures to Quiet.

Quiet before execution cancels stale scheduled delivery and prevents Wake Session creation for that morning. Global Pause suppresses eligible upcoming deliveries while preserving plans. `ringDespitePause` schedules the target morning normally while global Pause remains active for later mornings.

For MVP, explicit AlarmKit/system dismissal is treated as alarm acknowledgement equivalent to `I’m awake`, with acknowledgement source logged separately.

Follow-up alarms must respect boundaries:

```text
Fajr purpose: do not schedule follow-up alarms after Fajr ends.
Suhoor purpose: do not schedule follow-up alarms after Fajr begins.
```

## MVP Suhoor Alignment Addendum
This addendum is normative for MVP and supersedes conflicting lower sections in this file.

- Delivery consumes resolved Fajr/Suhoor alarm events plus Quiet/Pause alarm-state controls.
...
This addendum is normative for delivery and supersedes lower historical references to scheduling `Next 10` rows.

- Delivery must not schedule events merely because they appear in `Next 7 Mornings` or Weekly Fajrcast.
- `Next 7 Mornings` and Weekly Fajrcast may show the same seven visible dates, but that display horizon is still distinct from the active scheduled horizon.
- The delivery layer schedules only resolver-materialized events explicitly included in the active scheduled horizon or technical safety horizon.


## Wake Sessions / Wake Check Delivery Alignment Addendum

This addendum is normative for MVP follow-up-alarm delivery and supersedes any lower wording that treats snooze as the primary Subh wake-retry mechanism.

- A Wake Session materializes one primary wake alarm plus eligible follow-up-alarm alarms. Follow-up alarms are separate scheduled deliveries, not native snooze for MVP.
- Default core behavior is one primary alarm plus up to five follow-up alarms at five-minute intervals, for a maximum of six total wake attempts.
- Fajr follow-up alarms must not be scheduled later than five minutes before Fajr ends.
- Suhoor follow-up alarms must not be scheduled later than five minutes before Fajr begins.
- Each Wake Session delivery should carry or deterministically derive: `wakeSessionID`, date key, fire date, mode, role (`primaryWake` or `wakeCheck`), follow-up-alarm sequence number, sound role, and schedule signature.
- Platform identifiers for primary and follow-up-alarm alarms must be deterministic and scoped so stale follow-up alarms can be cancelled without affecting unrelated dates or events.
- Lock Screen actions for Wake Session alarms should expose `Stop` and `Open Subh`. The `Open Subh` action routes to the relevant in-app confirmation CTA.
- For MVP, explicit AlarmKit/system dismissal is treated as alarm acknowledgement equivalent to `I’m awake`; delivery must cancel remaining follow-up alarms for that morning while preserving acknowledgement source internally. If a later platform integration distinguishes temporary silence from dismissal, that distinction must be modeled explicitly before changing this behavior.
- When the user taps `I’m awake`, delivery must cancel any pending primary alarm and all remaining follow-up alarms for that Wake Session.
- Quiet is not exposed after the first alarm begins. When the user acknowledges the alarm with `I’m awake` or explicit system dismissal, delivery cancels all remaining primary/follow-up-alarm alarms for that session and records acknowledgement source.
- Mode changes, wake-time commits, time/location/timezone changes, and resolver boundary changes must cancel stale Wake Session alarm identifiers and schedule only the corrected eligible set.
- Reconciliation must detect missing, stale, duplicate, or mismatched follow-up-alarm deliveries, not only primary alarms.
- Advanced follow-up-alarm intervals, adaptive follow-up alarms, and heavy-sleeper escalation are out of this MVP delivery pass unless another approved spec explicitly requests them.
...
The delivery layer consumes resolver-materialized events that the parent morning/window builder marks as operational. It must not schedule events merely because they appear in Next 7 Mornings, Weekly Fajrcast, month browsing, or a day detail view.
...
6. The device time, timezone, location, calculation method, Fajr adjustment, date override, or wake purpose changes.
...
9. A later Fajr adhan cue is disabled but the before-Fajr wake is accidentally cancelled.
...
- Next 7 Mornings row layout or tag doctrine
...
| Next 7 Mornings | compact row forecast and tags | consumes schedule/delivery status only as resolved row state; no delivery diagnostics |
...
DeliveryReconciliation -> change wake purpose to Quiet
...
| before-Fajr wake | AlarmKit | yes, degraded | generic wake sound role unless resolver says otherwise |
...
- The delivery layer must not expand scope by reading calendar browsing data or Next 7 Mornings visible rows.
...
```text
Schedule all Next 7 Mornings rows because Next 7 Mornings is visible.
...
- the before-Fajr wake event remains active when the later Fajr adhan cue is toggled off;
...
- if the resolver materializes both before-Fajr wake and Fajr-start adhan, both are expected deliveries unless duplicate policy suppresses one;
...
### 24.4 Next 7 Mornings

Next 7 Mornings may use concise status if a wake time is unavailable or quiet.
...
- AND a stale before-Fajr wake delivery still exists on the platform
- WHEN delivery reconciliation runs
...
- GIVEN an Early + Fast non-Ramadan date has a before-Fajr wake event
- AND a later Fajr-start adhan cue is disabled for that date
- WHEN delivery planning runs
- THEN the before-Fajr wake delivery SHALL remain expected
...
- GIVEN Next 7 Mornings shows seven resolved days
...
- later Fajr-start cue toggle preserving before-Fajr wake;
...
10. Later Fajr-start adhan toggle off while before-Fajr wake remains active.
...
- [ ] Later Fajr adhan toggle does not cancel before-Fajr wake.
