...
| Version | 3 |
| Spec status | Product / design / engineering draft; aligned Desktop working spec; aligned to Next 7 Mornings horizon |
| Supersedes | None recorded in the active Desktop set |
| Related specs | `00-subh-spec-index-v3.md`, `subh-morning-resolution-contract-state-ownership-spec-v3.md`, `subh-day-purpose-opportunity-resolution-spec-v1.md`, `subh-alarm-delivery-schedule-reliability-spec-v3.md`, `subh-quick-wake-mode-intent-mutation-contract-v2.md`, `subh-wake-sessions-wake-checks-morning-logs-spec-v1.md`, `subh-quiet-mode-quiet-morning-contract-spec-v1.md`, `subh-sound-alarm-settings-spec-v1.md` |
...
- Generated-vs-stored doctrine for browsed months, Next 7 Mornings, and future edits.
...
## May 29 Planning / Quiet / Pause Scope Alignment Addendum
...
Planning must distinguish date-level overrides from app-wide policy:
...
Quiet is a date-level alarm override. Pause is an indefinite app-wide wake-alarm policy. Future range-based silence is deferred and, if later added, should be modeled as `Quiet selected mornings`, not MVP Pause.

Displaying a date in Next 7, Month Planning, or Detail does not create a Wake Session and does not schedule follow-up alarms unless the active scheduled horizon includes that date.

At Fajr end, the Home Hero switches to the next morning, while completed morning logs remain historical records tied to the actual morning.
...
- Display horizons such as Next 7 Mornings and month browsing may show fasting opportunities, but opportunity-only dates must not become durable Suhoor intentions.
...
This addendum is normative for v3 and supersedes lower historical references to `Next 10` as the active Home forecast horizon.

- The Home near-term display horizon is `Next 7 Mornings`.
- `Next 7 Mornings` shows the next immediate alarm / next relevant morning plus the following six mornings.
...
- Future saved intentions hydrate into Home, Next 7 Mornings, Weekly Fajrcast, and delivery only when the canonical resolver/window builder brings the date into the relevant surface or active scheduled scope.
...
## Wake Sessions / Morning Logs Planning Alignment Addendum

This addendum is normative for how wake execution and immediate logs relate to planning, anchoring, and history.

- A Wake Session is not a durable future plan. It is generated from the resolved morning when that morning enters the active scheduled horizon.
- Displaying a morning in `Next 7 Mornings`, Weekly Fajrcast, or month browsing does not create a Wake Session and does not schedule follow-up alarms unless the active scheduled window includes that date.
- Future Suhoor, Fajr, and Quiet intentions remain anchored planning records until the resolver brings them into the active operational window.
- When Suhoor is selected for a current or future morning, the durable meaning is `fastingDayPlanned` / Suhoor fasting intention according to the existing anchoring rules. The later operational action `I’m awake` records `fastingIntentConfirmed` for the actual morning.
- `fastingDayPlanned`, `fastingIntentConfirmed`, and future `fastCompletionOutcome` are distinct. The first may be planned in advance; the second occurs in the morning; the third is a later-day completion record.
- `fajrWakeOutcome` and `fajrPrayerOutcome` are distinct. Awake confirmation does not imply prayer confirmation.
- MorningLogs are history/execution records. Once recorded, they remain tied to the actual local date/time and calendar context at the time of action; they must not silently move because a future Hijri/calendar adjustment changes planned observances.
- If a future anchored intention moves after a calendar adjustment, its eventual Wake Session should be generated from the moved resolved morning when it enters the active scheduled horizon.
- Quiet selected for a morning becomes a quiet suppression record/overlay for that scope. It does not become a missed-prayer record and does not delete the underlying anchored intention unless the user explicitly removes that intention through the appropriate planning surface.
...
- how month browsing and Next 7 Mornings views relate to the active schedule;
...
| Month browsing and Next 7 Mornings generation policy | Planning Horizon and Intention Anchoring | Views are generated from the canonical resolver and may be cached temporarily. |
...
- **Display horizon** means what the user can see, such as Next 7 Mornings or a month view.
...
2. **Subh can show more than it needs to schedule.**
   The Next 7 Mornings forecast or a future month can be visible without all of those days being active alarms.
...
|---|---|
| **Morning** | The user-facing before-Fajr / Fajr-centered experience for a given civil date. |
...
|---|---|---|
| Home display range | Next 7 Mornings | Configurable, likely still a weekly / seven-day horizon |
...
- Next 7 Mornings rows are not automatically scheduled just because they are visible;
...
A browsed month, a displayed Next 7 Mornings row, or a stored future observance intention is not platform-scheduled merely because it exists.
...
The Next 7 Mornings forecast can be resolved and shown. A future month can be browsed. But Subh should only activate events inside the canonical active scheduled window.
...
## 12. Next 7 Mornings Behavior

### 12.1 Purpose

The Next 7 Mornings card is a near-term view of the user's upcoming Fajr-centered days.
...
The Next 7 Mornings view should behave like a rolling weekly window.
...
Next 7 Mornings and Weekly Fajrcast must describe the same seven visible dates.
...
### 12.5 User changes in Next 7 Mornings

If the user changes a day reached from Next 7 Mornings, Subh must store the change as an anchored intention or override.
...
A day appearing in the Next 7 Mornings card does not mean Subh has permanently stored a record for that day.
...
- Ensure the change appears later in Next 7 Mornings when the date approaches.
...
Given a user opens Next 7 Mornings and makes no edits, Subh should not create durable day-specific records for each default day.
...
Given a day appears in Next 7 Mornings or month browsing, Subh should not schedule a platform delivery for that day unless the canonical active window explicitly includes a resolver-materialized event for that date.
...
2. Next 7 Mornings is generated for display from current rules.
...
#### Scenario: Next 7 Mornings shows future days

- GIVEN Next 7 Mornings displays seven days
...
- GIVEN the user opens Next 7 Mornings or a calendar month
...
#### Scenario: Future observance enters Next 7 Mornings

- GIVEN a user planned an observance fast from month browsing
- WHEN that date later enters Next 7 Mornings
- THEN the canonical resolver SHALL apply the stored anchor and show the intended fast
- AND Next 7 Mornings SHALL NOT infer the fast from tag text alone
...
- before-Fajr completion logging,
