...
## May 29 Shared Tag Alignment Addendum
...
Shared tags must not present routine wake purpose or alarm delivery states as opportunity/context tags.

Do not render these as middle-lane/context chips:
...
`Quiet`, `Paused`, `Rings once`, `Turn on alarms`, and `Alarm issue` belong in status/trailing/action areas, not opportunity tag lanes.
...
Wake-mode tags describe the resolved top-level user wake purpose.

MVP wake-purpose labels:
...
- `Suhoor` is the only exposed before-Fajr wake-purpose tag in MVP.
- Do not show `Pre-Fajr`, `Fast`, `Early`, `Tahajjud only`, or `Other early worship` as active MVP wake-purpose tags.
...
The active MVP wake-purpose vocabulary is:
...
```text
Pre-Fajr
Fast as top-level wake purpose
Early
Tahajjud only
Other early worship
Fasting + Tahajjud
...
- The wake purpose remains Fajr unless another resolved state applies.
...
- Use `[Suhoor]`, `[Fajr]`, and `[Quiet]` as MVP wake-purpose state chips unless the existing implementation is intentionally retained under a migration compatibility note.
...
Older Next 7 prose used `[Fasting]` as a compact planned-fast tag. For MVP Suhoor alignment, `[Fasting]` should be interpreted as a legacy planned-fasting presentation label and should not be used as the top-level wake-purpose tag. The preferred MVP compact wake-purpose tag is `[Suhoor]`.

### 8.2 Primary Morning Context compact

Rules:

- The context module may omit wake-purpose chips if the Hero selector already shows the mode.
...
- [ ] MVP active wake-purpose tags are `Suhoor`, `Fajr`, and `Quiet`.
