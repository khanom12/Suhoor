...
| Field | Value |
| --- | --- |
| Canonical filename | `subh-morning-resolution-contract-state-ownership-spec-v3.md` |
| Version | 3 |
| Spec status | Draft; reconciled Desktop working spec; aligned to Next 7 Mornings / Weekly Fajrcast horizon |
| Supersedes | `subh-morning-resolution-contract-state-ownership-spec-v2.md` |
| Related specs | `00-subh-spec-index-v3.md`, `subh-planning-horizon-day-resolution-intention-anchoring-spec-v3.md`, `subh-day-purpose-opportunity-resolution-spec-v1.md`, `subh-quick-wake-mode-intent-mutation-contract-v2.md`, `subh-alarm-delivery-schedule-reliability-spec-v3.md`, `subh-fajr-time-calculation-determination-selection-spec-v1.md`, `subh-wake-sessions-wake-checks-morning-logs-spec-v1.md`, `subh-quiet-mode-quiet-morning-contract-spec-v1.md`, `subh-sound-alarm-settings-spec-v1.md` |
...
## May 29 Quiet / Pause / Wake Purpose Resolution Alignment Addendum
...
The canonical resolved morning object must separate wake purpose from alarm state.
...
PurposeSpecificAlarmConfig: fajrAlarmConfig + suhoorAlarmConfig
DateAlarmOverride: none | quiet | ringDespitePause
GlobalWakeAlarmPolicy: active | pausedIndefinitely
ResolvedAlarmState: active | quiet | pausedInherited | ringsOnceDespitePause | blocked | issue | unavailable
WakeExecutionState: notStarted | scheduled | ringing | followUpPending | awakeAcknowledged | fastingLogged | fajrLogged | endedNoResponse | issue
```

Quiet and Pause must not overwrite wake purpose, day meaning, prayer-time context, or purpose-specific saved alarm settings.

Switching Fajr/Suhoor changes `WakePurpose` only. Quiet/Pause changes alarm delivery only.

## MVP Suhoor Alignment Addendum
This addendum is normative for MVP and supersedes conflicting lower sections in this file.

- The canonical exposed wake-purpose selector is `Fajr | Suhoor`; Quiet is a date-level alarm-state override, not a wake purpose.
- `Suhoor` is the only MVP before-Fajr user-facing wake purpose.
...
## v3 Next 7 Mornings / Weekly Fajrcast Alignment Addendum

This addendum is normative for v3 and supersedes lower historical references to `Next 10` as the active Home forecast surface.

- Surface snapshots must support `Next 7 Mornings`, not the archived historical `Next 10 Mornings`, as the active near-term forecast surface.
- `Next 7 Mornings` and Weekly Fajrcast consume the same seven resolved date keys in the same order.
- The first visible date is the next immediate alarm or next relevant morning supplied by the canonical resolver/window builder.
- The remaining six visible dates are the following six calendar mornings.
- Forecast visibility does not imply platform scheduling.
- The resolver/window builder must distinguish `visibleDateKeys` from `activeScheduledDateKeys` exactly as before.


## Wake Sessions / Morning Logs Alignment Addendum

This addendum is normative for wake execution and immediate morning logging. It preserves the existing one-morning resolver while adding Wake Session and MorningLog as operational records downstream of resolution.

- The canonical morning resolver owns day meaning, selected mode, wake boundary, resolved wake time, alarm activation, and materialized delivery events.
- The Wake Session owns execution state for one operational morning: primary alarm, follow-up alarms, awake confirmation, quiet cancellation, expiry, and reconciliation.
- MorningLog owns factual local records needed for reliability, restoration, and immediate user confirmations. It must not become a parallel day-purpose resolver.
- These records must remain separate: `suhoorWakeOutcome`, `fastingDayPlanned`, `fastingIntentConfirmed`, `fajrWakeOutcome`, `fajrPrayerOutcome`, and future `fastCompletionOutcome`.
- Selecting Suhoor sets `fastingDayPlanned` for the resolved morning. Tapping `I’m awake` confirms Suhoor wake and sets `fastingIntentConfirmed` for that morning.
- Tapping `I’m awake` confirms Fajr awake status only. It must not mark Fajr prayer as prayed.
- Tapping `I prayed Fajr` confirms Fajr prayer outcome only. It must not invent or rewrite Suhoor, fasting, or wake outcomes except where the user action explicitly says so.
- Lack of awake confirmation may produce `expiredUnconfirmed` or `unconfirmed`. It must not automatically produce `Fajr missed`, `Fast missed`, or prayer judgment.
- Quiet remains an intentional suppression overlay. It may log `quietMorning`, cancel wake execution, and preserve underlying Fajr/Suhoor meaning, but it must not log a missed prayer or missed fast.
- Wake Session state must be recomputed or reconciled after mode changes, wake-time changes, date rollover, app relaunch, timezone/location changes, and delivery-ledger discrepancies.
- Wake Sessions and local operational MorningLogs are core system records for all users. Long-term analytics, streaks, adaptive recommendations, and advanced reporting remain separate future/paid surfaces unless another spec changes that scope.

## 0. One-page summary

Subh already has a clear product doctrine: it is a **Fajr-centered morning system**. Suhoor, fasting, Ramadan, Qada, observance opportunities, Quiet, and one-day edits are not separate engines. They are layered states over the same morning-resolution system.

This specification turns that doctrine into an implementation contract.

For any morning date, Subh must produce one canonical resolved morning object graph. Home Hero, Alarm Detailed View, Weekly Fajrcast, Next 7 Mornings, alarm scheduling, notification copy, completion logic, and analytics must consume that resolved graph instead of independently re-deriving prayer windows, wake purposes, day meaning, or alarm state.
...
Quiet can suppress delivery without deleting the underlying Fast or Fajr mode.
...
How does Quiet preserve underlying state?
How do prayer windows, day purpose, wake purposes, scheduling, and UI snapshots stay synchronized?
```
...
1. The Home Hero decides one wake purpose while Alarm Detail decides another.
2. Weekly Fajrcast plots one wake marker while the scheduler materializes another wake event.
3. Next 7 Mornings rows infer `[Fasting]` from tags even though the user has not intended a fast.
4. Quiet deletes the prior mode instead of suppressing delivery as an overlay.
...
- Next 7 Mornings row grid details
...
| Weekly Fajrcast v14 | upcoming seven-day chart rendering and interaction | rule that chart consumes resolved snapshots and live preview values only |
| Next 7 Mornings v1 | seven-row visual/tag contract | rule that rows consume resolved tags and must not infer intention locally |
| Early Worship Boundary v2 | final-third calculation and boundary semantics | ownership of final-third resolver in the canonical pipeline |
...
8. **Next 7 Mornings** — seven-row forecast layout, tags, date alignment with Weekly Fajrcast, and accessibility.
...
- Quiet suppresses wake delivery without deleting the underlying morning meaning.
...
Next 7 Mornings row → infers fasting intention from tag text
...
- `visibleDateKeys` powers surfaces such as Next 7 Mornings and month browsing.
...
| Supported current/planning range | Planning Horizon / window builder | resolvers, surfaces, scheduler handoff | views hard-coding the Home forecast horizon as full product range |
...
| Fajr ends | prayer-time resolver | Hero, Fajrcast, Next 7 Mornings, scheduler | renderer-invented offset |
...
| `fast` | user intends fasting / before-Fajr wake | 30 min before Fajr begins | finalThirdStart → Fajr begins |
...
| `fajrAdhanAtFajrBeginsOverride` | detail audio toggle handler | only later Fajr adhan event, not before-Fajr wake alarm |
...
- `fajrBoundaryNotice` is separate from the before-Fajr wake alarm.
...
Turning off later Fajr adhan = does not disable the before-Fajr wake alarm.
```
...
### 17.4 Next 7 Mornings

Input:

```text
NextSevenDaysSnapshot generated from resolved day/wake states
row date labels
resolved tag list
...
- The seven row date keys must match Weekly Fajrcast visible date keys exactly and in the same order.
- Row 1 is the next immediate alarm or next relevant morning supplied by the canonical resolver/window builder.
...
AND Next 7 Mornings may show [Fajr] [White Days]
```
...
AND the before-Fajr wake alarm remains active
...
### 20.21 Next 7 Mornings tag doctrine

```gherkin
GIVEN an opportunity-only Ashura morning
WHEN Next 7 Mornings row tags are resolved
...
```gherkin
GIVEN Next 7 Mornings and a month view display dates outside the active scheduled horizon
...
- Next 7 Mornings snapshot tag rules
...
- Quiet moon icon and stable primary row slot
- Next 7 Mornings row tags without explanatory subtitles
...
6. Historical selector-label conflict is resolved for MVP: Home and Detail expose `Fajr | Suhoor`; Quiet is exposed through the alarm-state control; Fast/Early/Tahajjud-only wording is legacy or deferred unless a future spec reintroduces it.
...
2. Home Hero, Alarm Detail, Weekly Fajrcast, and Next 7 Mornings consume resolved snapshots rather than duplicating domain logic.
