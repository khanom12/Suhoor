...
## May 29 Context Presentation Alignment Addendum
...
Primary context copy must remain about the morning's meaning and opportunities, not alarm delivery internals.

Quiet/Paused alarm states may be visible in the Hero alarm-state slot and forecast trailing/status lanes, but context tags must not become routine `Quiet mode`, `Pause mode`, `Fajr`, or `Suhoor` tags.

Use `Suhoor` for the before-Fajr fasting-oriented wake purpose. Do not expose `Pre-Fajr`, `Early`, or `Fast mode` labels.
...
- Quick wake-purpose mutation rules.
...
Home compact tap MUST NOT directly mutate wake purpose, fasting purpose, or Quiet state.
