...
## May 29 Quiet / Pause / Fixed Hero State Alignment Addendum
...
The Home Hero is a fixed-height state machine with six fixed slots:
...
The hero frame height, baselines, text sizes, slot heights, and vertical spacing must remain fixed across all states.

### Selector and alarm-state control

The planning selector is now only:

```text
Fajr | Suhoor
```

Quiet is removed from the selector. Quiet and Pause are controlled through Slot 3, the alarm-state button/status.

### Final hero state table
...
| Fajr begins after Suhoor | This morning | `Fajr has begun` | locked/current morning marker | `Fajr ends at 6:31 AM` | `[ I prayed Fajr ]` when eligible |
...
During active alarm execution, Slot 6 contains only `[ I’m awake ]`. Do not show `I’m awake`, Quiet, or the Fajr/Suhoor selector during the active alarm/follow-up alarm flow.

`I’m awake` stops the active alarm flow and cancels remaining follow-up alarms for that morning.

### Post-awake CTAs

After `I’m awake`, wait at least 60 seconds before showing another completion CTA.

- Fajr purpose: show `[ I prayed Fajr ]` only after Fajr has begun, the delay has passed, and Fajr has not ended.
- Suhoor purpose: show `[ I’m fasting today ]` after Suhoor acknowledgement and delay, before Fajr begins; after Fajr begins, prioritize `[ I prayed Fajr ]`.
...
- The Home hero quick selector is `Fajr | Suhoor`.
- `Suhoor` is the only exposed before-Fajr wake purpose in MVP.
- The hero must not expose `Tahajjud only`, `Other early worship`, or generic non-fasting before-Fajr flows as selectable modes or intentions.
...
- Quiet remains intentional one-morning alarm suppression and must preserve the underlying Suhoor or Fajr state for restoration.
...
This addendum is normative for the current MVP wake-execution pass and supersedes conflicting lower hero wording only for awake acknowledgement, follow-up alarms, immediate morning logs, and active-window CTAs.
...
- After the primary alarm or a follow-up alarm has fired, and before awake acknowledgement, the slot replaces the adjuster with the prominent mode-specific CTA: `I’m awake`.
- `I’m awake` acknowledges the alarm and cancels remaining follow-up alarms for that morning. It must not mark `I prayed Fajr`.
- For Suhoor, `I’m awake` acknowledges the Suhoor alarm. `I’m fasting today` remains a separate delayed CTA and must not mark fast completion.
- After Fajr begins, a Suhoor user who already acknowledged the Suhoor alarm should see `I prayed Fajr` as the primary next completion CTA when eligible; the hero must not show a second `I’m awake` prompt at Fajr begins.
- `I prayed Fajr` remains a separate user action and separate log outcome from both Fajr awake acknowledgement and Suhoor awake acknowledgement.
- Quiet is not available after the first alarm begins. During active alarm execution, the user-facing action is `I’m awake`.
- The Hero may emit user intents such as `selectWakePurpose(.fajr | .suhoor)`, `setDateAlarmOverride(.quiet)`, `confirmAwake`, `confirmFastingToday`, and `confirmFajrPrayer`, but it must not directly schedule or cancel platform alarms.
- Lock Screen `Open Subh` actions should route the user to the relevant hero state. For MVP, explicit AlarmKit/system dismissal is treated as alarm acknowledgement equivalent to `I’m awake`, while preserving acknowledgement source internally.
- The active MVP selector label set is `Fajr | Suhoor`. Legacy `Pre-Fajr`, `Fast`, or `Early` examples retained lower in this document are historical or compatibility wording unless a later approved spec reintroduces them.
...
The Morning Hero is an **immediate-commit surface** for the next relevant morning. It may preview UI changes while the user taps, drags, or scrubs, but committed user actions must be expressed as normalized wake-purpose / intention mutation commands and then re-resolved through the canonical morning-resolution pipeline.
...
- Historical reset-on-mode-switch behavior is superseded by the May 29 alignment: switching `Fajr`/`Suhoor` preserves purpose-specific alarm settings, and Quiet is applied separately through the alarm-state control.
...
[ Fajr | Suhoor selected ]
```

Main changes retained and clarified in v15:

1. Align the Home hero and Alarm Detailed View mode labels to `Fajr | Suhoor`.
...
6. `Suhoor` uses the default wake rule: 30 min before Fajr begins.
7. When `Suhoor` is selected outside Ramadan, the Home hero shows or exposes a Pre-Fajr intention selector with `Tahajjud only`, `Fasting`, and `Other early worship`.
8. Outside Ramadan, `Suhoor` defaults to `Tahajjud only`.
...
11. During Ramadan, the default hero state is `Suhoor` + `Fasting` + `Ramadan fast`.
12. During Ramadan, the user cannot change the Pre-Fajr intention or fasting intention; they may switch to `Fajr` or `Quiet`.
13. When the user returns to `Suhoor` during Ramadan, the hero restores `Fasting` + `Ramadan fast` automatically.
14. On Eid days, fasting is unavailable; selecting `Suhoor` lands on `Tahajjud only`.
...
26. The `Fajr | Suhoor` selector must use the same translucent liquid-glass surface language as the Next 7 Mornings card, not an opaque or frosted gray segmented-control treatment.
27. Preserve the approved selector-highlight glide between `Suhoor`, `Fajr`, and `Quiet`.
...
31. Switching to or from `Quiet` must not cause the hero stack to jump vertically; `[moon icon] Quiet` replaces the primary wake time inside the same visual slot.
...
38. Permission or delivery failure must not be rendered as `Quiet`.
...
- **Wake quick mode:** the user's immediate target-morning choice: `Suhoor`, `Fajr`, or `Quiet`.
- **Pre-Fajr intention:** when `Suhoor` is selected, the user’s intention is `Tahajjud only`, `Fasting`, or `Other early worship`.
- **Fasting intention:** when `Suhoor` + `Fasting` is selected, the app resolves the applicable fasting intention: Ramadan, opportunity-based Sunnah fasts, Voluntary, Qada, Vow, Kaffarah, or `Other fast`, depending on date and user override rules.
...
Conditional structure when `Suhoor` is selected:
...
{quickWakeStateSelector: Fajr | Suhoor}
...
- In `Quiet` mode, the primary row should say exactly `Quiet`, with the moon icon when the app's icon set supports it.
- `Quiet` is centered in the same primary wake row slot used by active wake times.
- `Quiet` uses the same primary-row typography treatment as the active wake time: same optical size family, weight direction, line-height behavior, alignment rules, and dynamic type scaling.
- The primary row container must keep the same settled height across `Suhoor`, `Fajr`, and `Quiet` so the hero does not jump vertically when the mode changes.
...
- Use `interactiveEarlyWorshipWindow` when the resolved day has selected `Suhoor`, intended fasting, intended Tahajjud-only, or intended Other early worship and the active wake time or planned wake anchor is inside the final-third start → Fajr begins window.
...
- Selector-triggered changes between `Suhoor`, `Fajr`, and `Quiet` are not treated as abrupt data swaps. They must follow the quick-mode transition rules in Section 7.8.
...
- Use these patterns only when the resolved day has selected `Suhoor`, intended fasting, intended Tahajjud-only, or intended Other early worship and the active wake time is inside the final-third start → Fajr begins window.
...
- The primary wake row should say exactly `Quiet`.
...
- When the text changes because the user selects `Suhoor`, `Fajr`, or `Quiet`, change the line with a restrained fade-through or direct text content transition.
...
- Do not let the line's row height collapse or expand during ordinary `Suhoor`, `Fajr`, and `Quiet` transitions.
...
The quick wake-state selector lets the user quickly choose the target morning's main wake purpose without opening a deeper editor.
...
- The pill contains three equal logical segments: `Suhoor`, `Fajr`, and `Quiet`.
- `Suhoor` is on the left.
...
|---|---|---|---|
| `Suhoor` | The user wants a wake before Fajr begins. | Set active wake to 30 min before Fajr begins. | Use early-worship mode when final-third start and Fajr begin are available. |
...
- Ramadan mornings default to `Suhoor`.
- Selecting `Suhoor` sets the default wake to 30 min before Fajr begins.
- Outside Ramadan, selecting `Suhoor` defaults the Pre-Fajr intention to `Tahajjud only`.
- Outside Ramadan, the user may change the Pre-Fajr intention to `Fasting` when fasting is available, or to `Other early worship` for a non-fasting before-Fajr wake that is not represented by `Tahajjud only`.
- On Eid days, fasting is unavailable and selecting `Suhoor` defaults to `Tahajjud only`; `Other early worship` may remain available as a non-fasting early-worship reason only if product-approved for that context.
- During Ramadan, selecting `Suhoor` restores locked `Fasting` + `Ramadan fast`; `Tahajjud only` and `Other early worship` are not selectable while Ramadan owns the Pre-Fajr intention.
- Selecting `Fajr` clears the active Pre-Fajr wake purpose for the target morning unless another explicit calendar-locked state owns that day; the previously selected Pre-Fajr intention is preserved for restoration when allowed, but active manual wake-time adjustment is cleared.
...
- The `Quiet` primary row should say exactly `Quiet`.
- `Quiet` must occupy the same primary row slot used by the large wake time so the hero does not move vertically when Quiet is selected.
...
- Dragging an interactive adjuster after choosing `Suhoor` or `Fajr` preserves the selected quick mode and changes only the wake time within that mode's allowed range.
...
When `Suhoor` is selected, the Home hero shows a compact intention control.
...
- Show or expose the Pre-Fajr intention control only when `Suhoor` is selected.
...
- During Ramadan, the intention control is locked to `Fasting` or hidden with clear locked Ramadan state; the user cannot change it to `Tahajjud only` or `Other early worship` while the selected mode is Ramadan `Suhoor`.
...
When the user selects `Suhoor` + `Fasting`, show the fasting intention state. When the user selects `Suhoor` + `Other early worship`, do not show the fasting-intention state because the selected Pre-Fajr intention is non-fasting.
...
- The user may switch out of `Suhoor` into `Fajr` or `Quiet`.
- If the user returns to `Suhoor`, the hero restores `Fasting` + `Ramadan fast`.
...
- The `Suhoor` action should announce that it wakes 30 min before Fajr begins.
...
- The selected Pre-Fajr intention should also be reflected in the hero accessibility summary when `Suhoor` is selected.
...
The currently approved behavior is the selector movement inside the `Fajr | Suhoor` container. Preserve that sliding selected-state behavior. The refinements below constrain the rest of the hero so text, time, and adjuster transitions stay stable and understandable.

#### 7.8.1 General transition principles

- Animate visible changes caused by selecting `Suhoor`, `Fajr`, or `Quiet`, but use the correct animation type for each row.
...
- Keep row frames stable across ordinary `Suhoor`, `Fajr`, and `Quiet` transitions.
...
- Use the larger of the outgoing and incoming measured row heights during the transition, and keep the settled row heights visually consistent across normal `Suhoor`, `Fajr`, and `Quiet` states.
...
When switching between `Fajr` and `Suhoor`, animate the visible time itself rather than replacing it with a simple fade.
...
- When moving from `Fajr` to `Suhoor`, the time rolls earlier/downward in clock time.
- When moving from `Suhoor` to `Fajr`, the time rolls later/upward in clock time.
...
- Crossfade the active wake time row into `[moon icon] Quiet`.
- Keep the primary row slot the same height as the active wake-time row.
- `Quiet` is centered in the row and uses the same primary-row typography treatment as the time.
...
- Crossfade `[moon icon] Quiet` into the resolved active wake time.
...
Moving from `Fajr` to `Suhoor` means the user is choosing an earlier wake. The motion should read as moving earlier in time.
...
Moving from `Suhoor` to `Fajr` means the user is choosing a later wake. The motion should read as moving forward in time.
...
Required behavior when switching from `Fajr` or `Suhoor` to `Quiet`:
...
Required behavior when switching from `Quiet` to `Fajr` or `Suhoor`:
...
- The primary row says exactly `[moon icon] Quiet`.
...
- Do not base the primary-row height solely on the natural height of `Quiet`, because that will make the hero visually shrink.
...
- Use one deliberate translucent liquid-glass segmented pill only for the `Fajr | Suhoor` quick wake-state selector.
...
| Quiet primary text, e.g. `[moon icon] Quiet` | primary wake row token | same optical typography treatment as the wake time; centered in the primary row slot |
...
- During quick-mode transitions, preserve a stable measured transition frame so the hero does not jump vertically between `Suhoor`, `Fajr`, and `Quiet`.
...
- In `Quiet`, the moon icon and `Quiet` text should be centered as one group in the same primary row slot.
- `Quiet` must not use a smaller natural row height that pulls the hero upward.
...
- The primary wake row should say exactly `[moon icon] Quiet` when the moon icon is available, or `Quiet` when icons are unavailable.
- `Quiet` replaces the large wake time inside the same primary row slot; it must not cause the hero stack to jump upward.
- `Quiet` uses the same primary-row typography treatment as the active wake time and remains centered in the row.
...
- Do not show `Quiet` for permission failure, delivery failure, missing pending platform request, location failure, or unavailable Fajr data. Those are degraded, blocked, or unavailable states, not user-selected Quiet.
...
- Use the early-worship adjuster only when the resolved day has selected `Suhoor`, intended fasting, intended Tahajjud-only, or intended Other early worship.
...
- Outside Ramadan, `Suhoor` defaults to `Tahajjud only`.
...
- `Suhoor` + `Fasting` marks the target morning as intended fasting unless Ramadan already owns that state.
...
- It must remain distinct from `Other fast`, which appears only inside the fasting-intention taxonomy after `Suhoor` + `Fasting` is selected.
...
- Ramadan defaults to `Suhoor`.
- Ramadan locks intention to `Fasting`.
- Ramadan locks fasting intention to `Ramadan fast`.
- The user may switch to `Fajr` or `Quiet`, but returning to `Suhoor` restores the locked Ramadan state.
...
- The primary wake row says `[moon icon] Quiet` when the moon icon is available, or `Quiet` when icons are unavailable.
- The primary row keeps the same vertical slot, typography treatment, and container height as active wake purposes; no hero jump is allowed.
...
- Use the early-worship adjuster for intended fasting, `Suhoor` quick-select mode, intended Tahajjud-only mornings, or intended Other early worship mornings when final-third start and Fajr begins are available.
...
- The location line remains a location line; do not use it for special-context labels. Use the quick wake-state selector to show the selected `Suhoor`, `Fajr`, or `Quiet` state.
...
- A tap on `Suhoor`, `Fajr`, or `Quiet` emits `selectQuickWakeMode(...)` for the current target morning.
...
- `selectedQuickWakeMode = preFajr` when the target morning is selected as Pre-Fajr, resolved as Ramadan Pre-Fajr, or otherwise resolved as a before-Fajr wake for this surface.
...
- The renderer may keep the previous visible snapshot in presentation state to animate `Suhoor`, `Fajr`, and `Quiet` transitions. The authoritative selected mode, wake time, boundary kind, and relation text still come from the shared resolver.
...
- Selecting `Suhoor` should produce an active wake default of 30 min before Fajr begins. Outside Ramadan the default Pre-Fajr intention is `Tahajjud only`; during Ramadan the locked intention is `Fasting` with `Ramadan fast`. If the user selects or restores `Other early worship`, the resolved state remains non-fasting and uses the early-worship window.
- Selecting `Fajr` should produce an active wake default of 30 min before Fajr ends. Per the Quick Wake contract, changing the main quick wake purpose clears active manual wake-time adjustment unless a later approved spec explicitly changes that rule.
- Selecting `Quiet` should suppress the active wake alarm by user choice and set `wakeDisplayText` to `Quiet`; permission failure, platform delivery failure, or degraded reliability must not be rendered as Quiet.
...
```text
East York. Tomorrow. Quiet selected. No alarm will ring for tomorrow. Fajr begins at 4:43 AM. Fajr ends at 6:13 AM.
```

Rules:

- The summary should include the location used for the displayed Fajr times.
- The summary should include the selected quick mode when the quick wake-state selector is visible: `Suhoor purpose selected`, `Fajr mode selected`, or `Quiet selected`.
...
16. The quick wake-state selector is a translucent liquid-glass segmented pill with three states in this order: `Suhoor`, `Fajr`, `Quiet`.
...
20. `Suhoor` sets the target morning to a before-Fajr wake, defaults to Tahajjud only outside Ramadan, exposes/restores `Fasting` and `Other early worship` when applicable, and sets the default wake to 30 min before Fajr begins.
21. `Quiet` suppresses the active wake alarm and shows `[moon icon] Quiet` as the primary wake row when the moon icon is available.
...
43. Switching to or from `Quiet` must keep the hero vertically stable; `[moon icon] Quiet` replaces the primary wake time inside the same row slot.
...
- [ ] Segment order is `Suhoor`, `Fajr`, `Quiet`.
...
- [ ] `Suhoor` selection sets wake to 30 min before Fajr begins, defaults to Tahajjud only outside Ramadan, exposes/restores Fasting and Other early worship when applicable, and uses early-worship mode when available.
- [ ] `Quiet` selection shows `[moon icon] Quiet`, suppresses the alarm, keeps the primary row height stable, and removes the alarm handle from the range visual.
...
- [ ] Quick wake-state selector exposes selected/unselected state for `Suhoor`, `Fajr`, and `Quiet`.
