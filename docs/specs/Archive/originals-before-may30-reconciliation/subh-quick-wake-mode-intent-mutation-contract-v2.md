...
| Field | Value |
| --- | --- |
| Canonical filename | `subh-quick-wake-mode-intent-mutation-contract-v2.md` |
| Version | 2 |
| Spec status | Persistent shared interaction contract; canonical Desktop working spec; aligned to Next 7 Mornings horizon |
| Supersedes | Historical/superseded: `subh-quick-wake-mode-intent-mutation-contract-v1.md` |
| Related specs | `00-subh-spec-index-v3.md`, `subh-morning-resolution-contract-state-ownership-spec-v3.md`, `subh-planning-horizon-day-resolution-intention-anchoring-spec-v3.md`, `subh-morning-hero-item-spec-v15.md`, `subh-alarm-detail-view-screen-spec-v7.md`, `subh-next-7-mornings-wake-forecast-spec-v2.md`, `subh-weekly-fajrcast-card-spec-v14.md` |
| Owning domain / surface | Shared wake-purpose and intention mutation behavior |
| Implementation audit status | Needs implementation audit |

## Purpose
Define the shared mutation contract for Fajr/Suhoor wake-purpose selection, Quiet alarm override, fasting intention, alarm adjustment, audio, immediate saves, reset behavior, and cross-surface consistency.

## What This Spec Owns
- Wake-purpose semantics and state transitions.
...
## May 29 Wake Purpose / Alarm State Mutation Alignment Addendum
...
The previous quick-mode model is superseded for MVP UI.

The Home/Detail selector now mutates only:

```text
WakePurpose: Fajr | Suhoor
```

Quiet is no longer selected through the wake-purpose selector. It is applied through the alarm-state button/action sheet as:

```text
DateAlarmOverride.quiet
```

Pause is not a quick mode. It is applied from Settings as:

```text
GlobalWakeAlarmPolicy.pausedIndefinitely
```

While Pause is active, `Ring tomorrow only` / `Ring this morning only` creates:

```text
DateAlarmOverride.ringDespitePause
```

Purpose-specific alarm settings must persist separately:
...
Switching Fajr/Suhoor must not reset the other purpose's saved/custom alarm configuration.

## MVP Suhoor Alignment Addendum
This addendum is normative for MVP and supersedes conflicting lower sections in this file.

The only exposed wake-purpose selector values are:

```text
Fajr | Suhoor
```

`Suhoor` replaces the older exposed `Fast`, `Pre-Fajr`, and `Early` labels. It is the only MVP user-facing before-Fajr wake purpose. It means: wake before Fajr begins for suhoor/fasting.
...
- Selecting `Suhoor` sets the date-specific wake purpose to the before-Fajr wake regime.
...
- Supported explicit fasting-purpose overrides such as Qada, Vow/Nadhr, Kaffarah, and Other fast remain fasting-purpose choices, not separate wake purposes.
...
## v2 Next 7 Mornings / Weekly Fajrcast Alignment Addendum

This addendum is normative for v2 and supersedes lower historical references to `Next 10`.

- The shared mutation contract treats `Next 7 Mornings` as the active Home forecast route/list surface.
- Expanding, collapsing, scrolling, or inspecting `Next 7 Mornings` does not mutate wake state.
- Tapping a Next 7 Mornings row routes to Day Detail; the mutation occurs only in the detail/editor surface.
- Weekly Fajrcast uses the same seven visible dates but remains inspection-only and must not mutate wake state.
- Future saved mutations hydrate when the date enters Next 7 Mornings, Weekly Fajrcast, Home, or the active scheduled horizon through canonical Morning Resolution.

## 0. One-page summary

This specification defines how Subh mutates a user's wake purpose for one target morning when the user selects:

```text
Fajr | Suhoor
```

It also defines how that wake purpose interacts with:
...
It exists to prevent Home Hero, Alarm Detailed View, Next 7 Mornings, Month Browsing, Adjusted Days, Morning Resolution, and Alarm Delivery from each inventing their own version of the same state transition.
...
```text
Quick wake purpose ≠ fasting intention ≠ manual wake adjustment ≠ Quiet suppression ≠ delivery permission.
```

Examples:

```text
Suhoor is the top-level before-Fajr wake purpose for MVP.
...
9. A far-future edit is saved but does not hydrate Home or Next 7 Mornings later.
...
- canonical quick wake purpose terminology;
...
- Day Detail immediate-save and Done-exit behavior;
...
Next 7 Mornings mutates wake state directly from row tags.
...
For MVP, this document intentionally absorbs the minimum needed rules for both areas because they are tightly coupled to quick wake purpose selection.
...
| Source | Relationship |
|---|---|
| `00-subh-spec-index-v3.md` | Canonical terminology and no-functionality-loss guardrail. |
| `subh-mvp-interaction-inventory-v4.md` | MVP scenario authority and scenario IDs. |
| `subh-morning-resolution-contract-state-ownership-spec-v3.md` | Parent owner of canonical resolved morning graph. |
| `subh-planning-horizon-day-resolution-intention-anchoring-spec-v3.md` | User meaning, durable future edits, generated-vs-stored doctrine. |
| `subh-day-purpose-opportunity-resolution-spec-v1.md` | Opportunity vs intention vs completion vs credit separation. |
| `subh-early-worship-boundary-spec-v2.md` | Final-third boundary for intended early worship. |
| `subh-fajr-time-calculation-determination-selection-spec-v1.md` | Prayer-window boundaries consumed by this contract. |
| `subh-alarm-delivery-schedule-reliability-spec-v3.md` | Platform scheduling and reliability after resolution. |
...
| `subh-next-7-mornings-wake-forecast-spec-v2.md` | Seven forecast rows that align with Weekly Fajrcast, route into Day Detail, and hydrate after edits. |
| `subh-weekly-fajrcast-card-spec-v14.md` | Chart surface that consumes resolved wake state only. |
...
| K | S089–S109 | Day Detail editing rules at the mutation boundary, including immediate save, Done exit, and Reset to Defaults. |
...
### 5.1 Quick wake purposes

Canonical user-facing quick wake purposes:
...
|---|---|---|
| `Early` | `Suhoor` compatibility path | Load as Suhoor-compatible before-Fajr wake state; display `Suhoor` in active MVP UI. |
| `Fast` as a quick mode | `Suhoor + fasting intention` | Split into the Suhoor mode plus fasting intention metadata. |
...
### 5.2 Historical/deferred Pre-Fajr intentions

Historical Pre-Fajr intention set, preserved for migration/audit only:
...
|---|---|---|---|
| `Other early worship` | Non-fasting Pre-Fajr worship reason | `deferred Other early worship` | No fast completion requirement. |
| `Other fast` | Fasting intention outside named fast taxonomy | `Suhoor + Other fast` | Fast completion may apply. |
...
### 7.4 Immediate-save model for Day Detail

Alarm Detailed View may use transient presentation state, but MVP persistence is immediate:

```swift
struct MorningWakeIntentDraft: Equatable {
    var baseSnapshot: ResolvedDaySnapshot
    var pendingPresentationOverride: MorningWakeIntentOverride?
    var lastCommittedOverride: MorningWakeIntentOverride?
    var resetToDefaultsAvailable: Bool
}
```

The implementation may use a different presentation structure, but the behavior must be:

```text
Day Detail changes commit immediately.
Reset to Defaults in Day Detail commits immediately.
Leaving the screen must not discard a visible committed edit.
```

The `Done` action remains an exit affordance, not the persistence boundary.
...
| Alarm Detailed View quick mode tap | Immediate commit | Persists the selected date change immediately. |
| Alarm Detailed View Suhoor intention tap | Immediate commit | Persists the selected date change immediately. |
| Alarm Detailed View fasting intention change | Immediate commit | Persists the selected date change immediately. |
| Alarm Detailed View wake drag end | Immediate commit | Persists the selected date change immediately. |
| Alarm Detailed View Reset to Defaults | Immediate commit | Deletes the selected date override immediately. |
...
| Next 7 Mornings row tap | No mutation | Opens Day Detail. |
| Weekly Fajrcast scrub/tap | No mutation | UI-only inspection. |
| Month browsing day tap | No mutation until Day Detail edit | Opens Day Detail for that date. |
...
```text
A fasting intention may be selected only when Suhoor is active and fasting is available for the date.
```

Requirement QW-026:

```text
On a non-Ramadan date with one or more fasting opportunities, selecting Suhoor SHALL default to the best opportunity-based voluntary fast according to Day Purpose rules.
```

Requirement QW-027:

```text
On a non-Ramadan date with no specific fasting opportunity, selecting Suhoor SHALL default to Voluntary fast.
```
...
```text
Changing fasting intention while Suhoor remains active SHALL NOT clear manual wake adjustment.
```
...
```text
Home Hero SHALL use canonical labels: Suhoor, Fajr, Quiet.
```
...
Alarm Detailed View is the immediate-save editor for one selected day.

Requirement QW-046:

```text
Alarm Detailed View SHALL load from the current resolved snapshot and any date-specific override for the selected morning.
```

Requirement QW-047:

```text
Alarm Detailed View SHALL save quick-mode, Suhoor intention, fasting intention, wake adjustment, and reset changes immediately for MVP.
```

Requirement QW-048:

```text
Tapping Done SHALL exit the screen; it is not the persistence boundary for MVP edits.
```

Requirement QW-049:

```text
Reset to Defaults in Alarm Detailed View SHALL become durable immediately when the user confirms or taps the reset action.
```

Requirement QW-050:

```text
After each committed edit, Alarm Detailed View SHALL trigger canonical morning re-resolution and schedule refresh for the affected date scope.
```

Requirement QW-051:

```text
Reopening Alarm Detailed View SHALL show the committed state.
```

Requirement QW-052:

```text
Reopening Alarm Detailed View after Reset to Defaults SHALL show the default resolved state for that date.
```
...
- Ramadan may still resolve to `Suhoor + Ramadan fast`.
...
```text
Delivery status SHALL NOT feed backward into quick wake purpose, Pre-Fajr intention, fasting intention, or Quiet state.
```
...
- a generated Next 7 Mornings row is displayed;
...
```text
When the future date later enters Next 7 Mornings, Home, or the active scheduled horizon, the saved mutation SHALL hydrate through canonical Morning Resolution.
```
...
```text
Expanding/collapsing Next 7 Mornings, Browse by Month, or Weekly Fajrcast SHALL NOT mutate wake state.
```
...
| Alarm Detailed View | Yes | Yes | Yes | Yes | Yes | Immediate | Selected date editor. |
| Next 7 Mornings | No direct mutation | No | No | No | No | N/A | Tap row opens Day Detail. |
...
```text
During Ramadan, the default resolved wake state SHALL be Suhoor + Ramadan fast unless the user selects Fajr or Quiet where product rules allow.
```
...
```text
A fasting opportunity SHALL NOT become a fasting intention unless the user selects Suhoor or Ramadan/default-obligatory rules require it.
```
...
```text
Legacy `Early` records SHALL load through the Suhoor compatibility path and display as `Suhoor` in active MVP UI.
```

Requirement QW-087:

```text
Legacy `Fast` quick-mode records SHALL load as `Suhoor + fasting intention`.
```

Requirement QW-088:

```text
Legacy non-fasting `Tahajjud` reason records SHALL be preserved as deferred historical data and must not re-expose Tahajjud-only as an active MVP quick mode.
```

Requirement QW-089:

```text
Legacy `Other` records under Early/Pre-Fajr SHALL be preserved as deferred historical data and must not re-expose Other early worship as an active MVP quick mode.
```
...
### 25.5 Suhoor without opportunity
...
```text
GIVEN the user selected Suhoor for a target morning
...
```text
GIVEN the user selected deferred Tahajjud-only
...
### 25.14 Day Detail immediate save
...
- [ ] Ensure Day Detail edits commit immediately for MVP.
- [ ] Ensure Reset to Defaults in Detail commits immediately for MVP.
...
- [ ] Alarm Detail uses this contract for immediate edits and reset commits.
- [ ] Next 7 Mornings rows do not mutate directly.
...
- [ ] Integration tests for Home → Next 7 Mornings → Day Detail consistency.
- [ ] Integration tests for Day Detail immediate edit, Done exit, and Reset to Defaults.
...
| Should Reset to Defaults in Day Detail commit immediately? | Yes for MVP. Older staged-reset behavior is superseded. | Revisit only with a later approved draft-editing spec. |
...
```text
Implement the Subh Quick Wake Mode and Intent Mutation Contract v2.
...
Canonical MVP modes are Suhoor, Fajr, Quiet. Early and Fast are legacy aliases that normalize to the Suhoor compatibility path. Tahajjud-only and Other early worship are deferred from active MVP UI and active MVP resolution unless a later approved spec reintroduces them. Other fast remains distinct from deferred Other early worship.

Implement idempotent mode selection, Suhoor intention preservation where valid, Quiet suppression/restoration, manual wake adjustment clearing on mode changes, Home immediate commits, Alarm Detail immediate edits, immediate Reset to Defaults, Ramadan lock, Eid fasting unavailable, and rapid final-state behavior.
...
Add tests for S028-S045, S056-S070, S089-S109, and S226-S235 from subh-mvp-interaction-inventory-v4.md, plus supporting tests for S046-S055 and S205-S211 where this contract interacts with Day Purpose and Alarm Delivery.
```
...
- [x] Keeps `Fajr | Suhoor` selector canonical, with Quiet as alarm-state override.
- [x] Treats `Early` as a legacy alias only.
- [x] Splits legacy `Fast` mode into `Suhoor + fasting intention`.
- [x] Defers `Tahajjud only` from active MVP UI/resolution while preserving historical data for audit/migration.
- [x] Defers `Other early worship` from active MVP UI/resolution while preserving historical data for audit/migration.
...
- [x] Defines Day Detail immediate-save edits for MVP.
- [x] Defines Reset to Defaults as immediate in Day Detail for MVP.
