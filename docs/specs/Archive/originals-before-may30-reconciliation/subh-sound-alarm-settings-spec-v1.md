...
## May 29 Sound / Pause Scope Alignment Addendum

Quiet and Pause control the Subh wake-alarm family, not every possible future notification unless a later spec explicitly broadens scope.
...
Avoid `Pause all alarms` unless the product deliberately includes every alarm/reminder category. For MVP, use `Pause Subh wake alarms` / `Alarms paused`.
