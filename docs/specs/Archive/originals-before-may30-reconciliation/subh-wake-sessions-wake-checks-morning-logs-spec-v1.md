...
**Canonical owner:** This spec owns wake execution, awake acknowledgement, follow-up alarms, and immediate morning logs. Other specs should reference this spec rather than duplicate its rules.
...
## May 29 Wake Flow / Hero CTA Alignment Addendum
...
### One-button wake execution

During active alarm execution, the user-facing Home Hero and in-app wake surface show one primary action:

```text
I’m awake
```

Do not expose a separate `I’m awake` button in MVP. `I’m awake` stops the current alarm and cancels remaining follow-up alarms for that morning.

For MVP, explicit system alarm dismissal from AlarmKit/system UI is treated as equivalent to `I’m awake`, while preserving an internal acknowledgement source:

```text
acknowledgedBy: inAppButton | systemAlarmDismiss
```

### Quiet and Pause timing

- Quiet before the first alarm begins cancels/suppresses the primary alarm and follow-up alarms for that morning.
- Quiet after the first alarm begins is not exposed; the action is `I’m awake`.
- Pause before a morning enters execution prevents Wake Session creation unless the date has `ringDespitePause`.
- Pause activated while a wake is already executing must not silently rewrite the current execution history; it applies to future alarms unless a later explicit active-session pause prompt is specified.

### Follow-up alarm boundaries

Follow-up alarms are allowed only while there is enough time before the relevant boundary:
...
If there is no room for a follow-up alarm, the hero shows `Final alarm this morning` and still offers `[ I’m awake ]`.

### Fajr and Suhoor post-awake flows

Fajr:

```text
I’m awake → wait 60 seconds → I prayed Fajr when Fajr has begun and not ended → Fajr complete → switch to next morning at Fajr end
```

Suhoor:

```text
I’m awake → wait 60 seconds → I’m fasting today before Fajr begins → Fajr event plays at Fajr begins → I prayed Fajr when eligible → switch to next morning at Fajr end
```

`I’m fasting today` records fasting intention/status for the day. It is not fast completion.
...
The Wake Sessions spec remains the canonical owner for wake execution, follow-up alarms, awake acknowledgement, and immediate MorningLog behavior. The new Quiet and Sound specs own their narrower domains and should be referenced rather than duplicating their rules here.


## 1. Purpose

Subh must keep alarm acknowledgement, Fajr prayer logging, and fasting status separate. For the current MVP, explicit AlarmKit/system dismissal is treated as an alarm acknowledgement equivalent to `I’m awake`, while preserving acknowledgement source internally. This spec defines a **Wake Session** model in which Subh supports the user through the primary alarm and eligible follow-up alarms until the alarm is acknowledged or the relevant morning boundary closes.
...
1. **Alarm acknowledgement ≠ Fajr complete.**
2. **You’re awake ≠ Fajr complete.**
3. **Suhoor acknowledged ≠ Fajr complete.**
4. **Suhoor mode implies a planned fasting day, but fasting completion remains a separate future record.**
5. **Quiet cancels wake execution but does not mean the user missed Fajr.**
...
| Term | Definition |
|---|---|
| **Wake Session** | The resolved operational wake plan for one local morning. Includes primary alarm, follow-up alarms, confirmation state, and cancellation/expiry state. |
| **Primary alarm** | The first scheduled AlarmKit alarm for the resolved wake time. |
| **Follow-up alarm** | A follow-up alarm scheduled after the primary alarm if the user has not confirmed awake. |
| **Awake acknowledgement** | User acknowledgement through in-app `I’m awake` or explicit system alarm dismissal for MVP. |
...
| **Fasting today logged** | User taps `I’m fasting today` after Suhoor alarm acknowledgement and before Fajr begins. This records current-day fasting status/intention, not fast completion. |
| **Quiet Morning** | A deliberate one-morning alarm suppression state. It is not a missed-prayer label. |
| **Hard cutoff** | The latest time at which Subh may schedule a follow-up alarm for that mode. |
...
|---|---|---|---|
| **Suhoor wake outcome** | User woke during the Suhoor/before-Fajr window. | `I’m awake` | No |
| **Fasting day planned** | User selected Suhoor mode for the morning. | Select/toggle Suhoor | Yes, when Suhoor is selected |
| **Fasting today logged** | User actively records current-day fasting status/intention after Suhoor. | `I’m fasting today` | No |
| **Fajr wake outcome** | User confirmed being awake during the Fajr prayer window. | `I’m awake` | No |
...
Subh must not infer `Fajr complete` from `I’m awake`, and must not infer `Fajr missed` from no confirmation.
...
Fajr mode is the default year-round wake purpose.

| Item | Rule |
|---|---|
| Default wake anchor | 30 minutes before Fajr ends, unless changed elsewhere by existing wake-resolution rules |
| Awake acknowledgement window | `Fajr begins <= now < Fajr ends` |
| Primary CTA | `I’m awake` |
| Prayer CTA | `I prayed Fajr`, after awake acknowledgement or where prior awake state is already valid |
| Wake-check hard cutoff | `Fajr ends - 5 minutes` |

### 5.2 Suhoor mode

Suhoor mode is the fasting-oriented before-Fajr wake purpose. It implies a planned fasting day.

| Item | Rule |
|---|---|
| Default wake anchor | 30 minutes before Fajr begins, unless changed elsewhere by existing wake-resolution rules |
| Suhoor awake acknowledgement window | `final third begins <= now < Fajr begins` |
| Suhoor CTA | `I’m awake` |
| Fasting day planned | Set when Suhoor mode is selected |
| Fasting intention confirmed | Set when user taps `I’m awake` |
...
| After Fajr begins, if Suhoor wake was not confirmed | Primary CTA becomes `I’m awake`; after that, `I prayed Fajr` |
...
| Item | Rule |
|---|---|
| Meaning | Do not ring alarms/follow-up alarms for this morning |
...
```text
Primary alarm + up to 5 follow-up alarms
Wake-check interval: 5 minutes
Maximum total wake attempts: 6
```

A follow-up alarm is scheduled only if all are true:
...
5:55 primary alarm
6:00 follow-up alarm 1
6:05 follow-up alarm 2
6:10 follow-up alarm 3
6:15 follow-up alarm 4
6:20 follow-up alarm 5
```

No follow-up alarm is scheduled at Fajr end.
...
4:40 primary alarm
4:45 follow-up alarm 1
4:50 follow-up alarm 2
4:55 follow-up alarm 3
5:00 follow-up alarm 4
5:05 follow-up alarm 5
```

No follow-up alarm is scheduled at Fajr begins.

### 6.4 Close-to-cutoff case

If the primary alarm is too close to the cutoff, Subh schedules only the primary alarm and any follow-up alarms that fit before the hard cutoff.
...
6:18 primary alarm
No 6:23 follow-up alarm because 6:23 is after the 6:20 hard cutoff.
```
...
| `scheduled` | Primary alarm and eligible follow-up alarms have been scheduled. |
...
| `wakeChecksPending` | One or more follow-up alarms remain scheduled. |
| `confirmedAwakeForSuhoor` | User tapped `I’m awake`. |
| `confirmedAwakeForFajr` | User tapped `I’m awake`. |
| `fajrPrayerConfirmed` | User tapped `I prayed Fajr`. |
| `expiredUnconfirmed` | Relevant window closed without awake acknowledgement. |
...
| User switches Fajr ↔ Suhoor | Cancel old alarm IDs, recompute mode windows and follow-up alarms, reschedule. |
| User taps `I’m awake` | Mark `confirmedAwakeForSuhoor`; mark/affirm `fastingIntentConfirmed`; cancel primary alarm if pending; cancel all remaining follow-up alarms. |
| User taps `I’m awake` | Mark `confirmedAwakeForFajr`; cancel primary alarm if pending; cancel all remaining follow-up alarms. |
...
| User stops an AlarmKit alarm | Stop only that alarm occurrence; do not mark awake; keep remaining follow-up alarms unless already cancelled. |
...
Quiet placement should follow the latest Quiet contract: it is an alarm-suppression control, not the same kind of wake target as Fajr/Suhoor.
...
```text
[I’m awake]
Follow-up alarms will continue until you confirm.
```
...
### 8.5 After awake acknowledgement

Fajr mode:

```text
You’re awake at {time}
[I prayed Fajr]
```
...
```text
[I’m awake]
Follow-up alarms for Suhoor have ended.
```

After `I’m awake`, the CTA may become:
...
Subh should not use a native snooze mechanic for the Wake Checks feature. Follow-up alarms are separate scheduled alarms owned by the Wake Session.
...
```text
Fajr follow-up alarm
Stop
Open Subh
```

```text
Suhoor follow-up alarm
Stop
Open Subh
```
...
For MVP, explicitly dismissing the Subh alarm from the Lock Screen is treated as alarm acknowledgement equivalent to `I’m awake`; it stops the current alarm and cancels remaining follow-up alarms for that morning. If a later AlarmKit integration distinguishes temporary silence from dismissal, that must be modeled separately.

Remaining follow-up alarms stay scheduled unless the user confirms awake, switches to Quiet, or changes/cancels the morning plan.
...
|---|---|
| Adaptive Wake Checks | Subh suggests increasing/decreasing follow-up alarms based on recent behavior. |
| Advanced interval control | User can choose intervals such as 2, 3, 5, or 10 minutes. |
| Advanced wake-check count | User can set max follow-up alarms beyond the default. |
...
If the current time is past the relevant cutoff, Subh must not schedule new follow-up alarms for that window.
...
|---|---|
| User wakes before primary alarm and taps `I’m awake` | Cancel primary alarm and follow-up alarms; mark Fajr awake confirmed. |
| User wakes before primary alarm and moves slider later | Recompute and reschedule the primary alarm/follow-up alarms if still within valid scheduling window. |
| User wakes for Suhoor and taps `I’m awake` | Cancel remaining Suhoor alarms; log Suhoor awake and fasting intention confirmed. |
| Suhoor user reaches Fajr begins after confirming Suhoor | Show `I prayed Fajr` as primary CTA. |
| Suhoor user reaches Fajr begins without confirming Suhoor | Suhoor wake expires unconfirmed; show `I’m awake` first. |
| User stops every alarm but never opens app | Wake session expires unconfirmed; do not mark Fajr missed. |
| User switches to Quiet during active follow-up alarms | Confirm first; then cancel alarms and log `quietMorning`. |
| Primary alarm is after hard cutoff | Schedule primary only if valid under existing alarm rules; no follow-up alarms beyond cutoff. |
| Fajr prayer is confirmed without prior awake acknowledgement through a future surface | Allow data model to store prayer confirmation independently; do not require data falsification. |
...
### 16.1 Fajr follow-up alarms

Given a Fajr morning with wake time 30 minutes before Fajr ends, when the Wake Session is scheduled, then Subh schedules one primary alarm and up to five follow-up alarms at five-minute intervals, with no follow-up alarm later than five minutes before Fajr ends.

### 16.2 Suhoor follow-up alarms

Given a Suhoor morning with wake time 30 minutes before Fajr begins, when the Wake Session is scheduled, then Subh schedules one primary alarm and up to five follow-up alarms at five-minute intervals, with no follow-up alarm later than five minutes before Fajr begins.

### 16.3 Alarm stop does not confirm awake

Given an alarm fires, when the user stops it from the Lock Screen, then the current alarm stops but the Wake Session remains unconfirmed and eligible remaining follow-up alarms stay scheduled.

### 16.4 Awake acknowledgement cancels remaining alarms

Given a Wake Session has pending alarms, when the user taps `I’m awake`, then Subh marks the relevant awake acknowledgement and cancels all remaining alarms for that morning.

### 16.5 Suhoor implies fasting context

Given the user selects Suhoor mode, then the MorningLog marks `fastingDayPlanned`. Given the user taps `I’m awake`, then the MorningLog marks `fastingIntentConfirmed`.

### 16.6 Prayer logging remains separate

Given the user taps `I’m awake`, then Subh must not mark `fajrPrayerConfirmed` until the user taps `I prayed Fajr`.
...
4. **Subh MVP Interaction Inventory**
   Add interactions for awake acknowledgement, prayer confirmation, Suhoor confirmation, Quiet during active session, alarm stop, and wake-check expiry.
...
7. **Quiet / Quiet Morning Contract**
   Align Quiet as alarm suppression that logs `quietMorning`, not missed prayer.
