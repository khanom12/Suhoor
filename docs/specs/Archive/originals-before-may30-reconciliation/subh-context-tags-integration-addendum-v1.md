...
## May 29 Quiet / Pause Context Integration Note

Context/tag integration must keep wake purpose and alarm state out of opportunity/context tags unless a specific surface explicitly owns that presentation.

`Quiet`, `Paused`, and `Rings once` are alarm status values. They belong in hero alarm-state controls, forecast trailing lanes, or Day Detail status controls, not ordinary context tags.
...
- The context summary must use `Fajr | Suhoor` wake-purpose vocabulary and `Quiet` only as an alarm-state/status value. Legacy `Pre-Fajr`, `Tahajjud only`, and `Other early worship` examples remain historical/deferred where contradicted by the MVP Suhoor addendum.
```
...
This addendum is normative for semantic tag meaning and supersedes lower tag examples only where they conflict with MVP `Fajr | Suhoor` wake-purpose vocabulary plus Quiet alarm-state vocabulary.
...
- Generic compact `[Fasting]` examples should be treated as legacy unless a later product decision deliberately keeps them. MVP compact mode should prefer `[Suhoor]` for the before-Fajr wake purpose and specific purpose/opportunity chips such as `[Arafah]`, `[Qada]`, `[Ramadan]`, or `[White Days]`.
...
- Scrubbing/inspection changes temporary focus only. It must not mutate context, tags, intention, or wake purpose.
...
The existing contract already owns `Fajr | Suhoor` wake-purpose mutation, Quiet preservation, and shared mutation routing. The new presentation specs consume the resolved result after this contract applies user intent.
