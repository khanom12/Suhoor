...
| Version | 4 |
| Spec status | Source-of-truth stabilized MVP inventory; pre-implementation-audit draft; aligned to Next 7 Mornings horizon |
| Supersedes | `subh-mvp-interaction-inventory-v3.md`; Archive/subh_mvp_interaction_inventory_v2.md |
| Related specs | `00-subh-spec-index-v3.md`, `subh-morning-resolution-contract-state-ownership-spec-v3.md`, `subh-quick-wake-mode-intent-mutation-contract-v2.md`, `subh-alarm-detail-view-screen-spec-v7.md`, `subh-morning-hero-item-spec-v15.md`, `subh-alarm-delivery-schedule-reliability-spec-v3.md`, `subh-wake-sessions-wake-checks-morning-logs-spec-v1.md`, `subh-quiet-mode-quiet-morning-contract-spec-v1.md`, `subh-sound-alarm-settings-spec-v1.md` |
...
## May 29 Interaction Inventory Alignment Addendum
...
The interaction inventory must add/update scenario coverage for:

1. Select Fajr purpose.
2. Select Suhoor purpose.
3. Quiet tomorrow from the Home Hero alarm-state button.
4. Turn alarm on from Quiet.
5. Switch Fajr/Suhoor while Quiet and preserve separate alarm settings.
6. Activate indefinite Pause from Settings.
7. Resume alarms from Pause.
8. Ring tomorrow only while Pause remains active.
9. Stay paused tomorrow after a ring-once exception.
10. Manual Quiet surviving global Resume.
11. Permission/setup blocked state not displaying as Quiet/Pause.
12. Active alarm showing only `I’m awake`.
13. System alarm dismissal treated as `I’m awake` for MVP.
14. Fajr `I prayed Fajr` delayed by at least 60 seconds and only visible during Fajr time.
15. Suhoor `I’m fasting today` delayed by at least 60 seconds and visible only before Fajr begins.
16. Fajr event after Suhoor showing Fajr prayer CTA, not a second awake CTA.
17. Follow-up alarms omitted near Fajr begins/end boundary with `Final alarm this morning` copy.
18. Fajr end handoff to next morning.

Core Quiet/Pause/alarm acknowledgement controls remain Free/core utility.
...
The active MVP wake-purpose selector is:

```text
Fajr | Suhoor
```

Quiet is managed as an alarm-state override, not as a selector option.

For MVP:
- Suhoor is the only user-selectable before-Fajr wake purpose.
...
- Alarm Detail saves wake-purpose changes and reset-to-default immediately. It does not require a staged Done-only save box.
...
## Next 7 Mornings / Weekly Fajrcast Alignment Addendum

This v4 addendum supersedes older historical inventory language that treated `Next 10 Mornings` as the active Home forecast surface.
...
Scenario impact:
- S074–S080 are renamed from collapsed historical `Next 10 Mornings` scenarios to collapsed `Next 7 Mornings` scenarios.
- S075 now expects seven upcoming days.
- Scenario IDs are preserved.


## Wake Sessions / Morning Logs Interaction Addendum

This addendum preserves the original `S001–S235` inventory and adds supplemental Wake Session traceability IDs. No existing scenario is removed, renumbered, or narrowed by this addendum.

Primary mapping: these supplemental scenarios refine Group Z (`S212–S219`) and also support Groups F, G, and AB where active wake execution intersects Quiet, wake-time adjustment, and rapid repeated interactions.

| Supplemental ID | Scenario | Owning spec |
|---|---|---|
| `WKS001` | Primary Fajr alarm fires; user explicitly dismisses it from Lock Screen. For MVP this is treated as `I’m awake`; remaining follow-up alarms are cancelled and acknowledgement source is recorded. | Wake Sessions; Alarm Delivery |
| `WKS002` | Primary Suhoor alarm fires; user explicitly dismisses it from Lock Screen. For MVP this is treated as `I’m awake`; remaining follow-up alarms are cancelled and acknowledgement source is recorded. | Wake Sessions; Alarm Delivery |
| `WKS003` | A follow-up alarm fires only when no alarm acknowledgement has been recorded and the relevant boundary still allows another alarm. | Wake Sessions; Alarm Delivery |
| `WKS004` | User taps `Open Subh` from an alarm or follow-up alarm and lands on the active Hero confirmation state. | Wake Sessions; Morning Hero |
| `WKS005` | User wakes before the primary Fajr alarm, opens Subh, and taps `I’m awake`; pending alarms are cancelled. | Wake Sessions; Morning Hero; Alarm Delivery |
| `WKS006` | User wakes before the primary Suhoor alarm, opens Subh, and taps `I’m awake`; pending Suhoor alarms are cancelled and fasting intention is confirmed. | Wake Sessions; Morning Hero |
| `WKS007` | User confirms Fajr awake, then separately taps `I prayed Fajr`. Awake and prayer outcomes remain separate records. | Wake Sessions; Morning Logs |
| `WKS008` | User confirms Suhoor awake; after Fajr begins the Hero shows `I prayed Fajr` as the next primary CTA. | Wake Sessions; Morning Hero |
| `WKS009` | User does not acknowledge the Suhoor alarm; after Fajr begins, the Hero shows the Fajr-begins/prayer state when eligible, not a second `I’m awake` prompt. | Wake Sessions; Morning Hero |
| `WKS010` | User stops every alarm and never opens Subh; the Wake Session expires unconfirmed without logging Fajr as missed. | Wake Sessions; Morning Logs |
| `WKS011` | During active alarm/follow-up flow, Quiet is unavailable; the Hero shows one action only: `I’m awake`. | Quiet; Wake Sessions |
| `WKS012` | User taps `I’m awake` during a follow-up alarm; remaining follow-up alarms are cancelled and acknowledgement source is recorded. | Wake Sessions; Alarm Delivery |
| `WKS013` | User chooses Quiet before the first alarm begins; scheduled primary/follow-up alarms for that morning are cancelled and the date is recorded as Quiet. | Quiet; Alarm Delivery |
| `WKS014` | User changes the wake time while a Wake Session is still valid; stale primary/follow-up-alarm alarms are cancelled and replaced. | Wake Sessions; Alarm Delivery |
| `WKS015` | User switches Fajr ↔ Suhoor for the target morning; old Wake Session deliveries are cancelled and mode-appropriate deliveries are scheduled. | Morning Resolution; Wake Sessions |
| `WKS016` | User reopens the app after termination or reboot; Subh reconciles MorningLog state with pending alarm identifiers. | Wake Sessions; Alarm Delivery |
| `WKS017` | Location/timezone/prayer-window change affects the active morning; Wake Session boundaries and pending deliveries are re-resolved. | Morning Resolution; Alarm Delivery |
| `WKS018` | Primary alarm is close to the mode cutoff; only follow-up alarms that fit before the hard cutoff are scheduled. | Wake Sessions; Alarm Delivery |
| `WKS019` | Basic local Wake Session logs exist for all users; long-term analytics and adaptive recommendations remain future/paid. | Wake Sessions; Pricing |
...
4. Main wake purposes are `Fajr | Suhoor`; Quiet is a one-morning alarm-state override.
...
22. Day Detail wake-purpose edits save immediately.
...
26. Future edits are saved and later hydrate Home, Next 7 Mornings, and Weekly Fajrcast when the date approaches.
...
5. Apply anchored intentions and date-specific overrides from Home, Day Detail, Next 7 Mornings, Month browsing, or Adjusted Days.
...
| I | S074–S080 | Collapsed Next 7 Mornings | Next 7 Mornings Spec; New Home Screen Composition Spec | Alarm Detail; Morning Resolution; Planning Horizon; Day Purpose; Weekly Fajrcast | Partially covered — Home composition missing | Next 7 Mornings card behavior exists; Home composition must own placement/collapse/routing and alignment with Weekly Fajrcast. |
| J | S081–S088 | Day Detail entry and viewing | Alarm Detailed View Spec | Home Composition; Next 7 Mornings; Month Browsing; Adjusted Days; Day Purpose | Partially covered — entry routes missing | Alarm Detail exists; missing surfaces must define routes into it. |
| K | S089–S109 | Day Detail editing | Alarm Detailed View Spec; New Quick Wake Mode Contract | Day Purpose; Alarm Delivery; Wake Adjustment Contract; Quiet Contract | Updated by MVP Suhoor addendum | Use Suhoor/Fajr/Quiet, defer Other early worship, and treat Reset to Defaults plus wake-purpose changes as immediate-save behavior. |
...
| P | S135–S143 | Monthly day list and future edits | New Month Browsing and Monthly Day List Spec | Planning Horizon; Morning Resolution; Alarm Detail; Alarm Delivery Reliability | Missing / partially covered | Define future override persistence, hydration into Home/Next 7 Mornings, and active scheduled horizon handoff. |
...
| AB | S226–S235 | Rapid and repeated interactions | New Quick Wake Mode Contract; Morning Resolution Contract | Morning Hero; Alarm Detail; Next 7 Mornings; Home Composition; Alarm Delivery Reliability | Partially covered — needs interaction stress tests | Define idempotency, rapid mutation ordering, final-state persistence, and no duplicate scheduling/state records. |

---

## 7. Scenario inventory

Each scenario below is preserved from v2/v3 and reconciled to current MVP terminology, including the Next 7 Mornings forecast horizon. Scenario IDs are intentionally unchanged.
...
### I. Collapsed Next 7 Mornings

Scenario IDs: `S074–S080`
Primary owning spec: Next 7 Mornings Spec; New Home Screen Composition Spec
...
Implementation status: **Not audited**
Completion needed: Next 7 Mornings card behavior exists; Home composition must own placement/collapse/routing.

S074. User sees collapsed Next 7 Mornings. It does not visually compete with the hero.
S075. User expands Next 7 Mornings. Seven upcoming days appear.
S076. User collapses Next 7 Mornings. No wake state changes.
S077. User scrolls/inspects expanded Next 7 Mornings. UI-only interaction.
S078. User taps a day in Next 7 Mornings. Day Detail opens for that date.
S079. User returns from edited Day Detail. The corresponding row updates.
S080. A visible day is edited from Next 7 Mornings. Home hero updates too when the edited day is the hero target.

### J. Day Detail entry and viewing

Scenario IDs: `S081–S088`
Primary owning spec: Alarm Detailed View Spec
Supporting specs: Home Composition; Next 7 Mornings; Month Browsing; Adjusted Days; Day Purpose
...
S082. User opens Day Detail from Next 7 Mornings.
...
Implementation status: **Not audited**
Completion needed: Use Suhoor/Fajr/Quiet, defer Other early worship, and keep Reset-to-Defaults plus wake-purpose edits immediate-save.
...
S099. User adjusts selected date wake time in Day Detail. Date-specific override saves immediately.
S100. User toggles adhan on in Day Detail. Adhan preference saves immediately for that date.
S101. User toggles adhan off in Day Detail. Adhan preference saves immediately for that date.
S102. User toggles adhan while Quiet is active. Adhan is shown as inactive or suppressed.
S103. User presses Done. The previous screen refreshes from already committed selected-day state.
...
Spec coverage status: **Missing / partially covered**
Implementation status: **Not audited**
Completion needed: Define future override persistence, hydration into Home/Next 7 Mornings, and active scheduled horizon handoff.

S135. User views monthly day list. Rows show date, wake time, and tags.
S136. User taps a monthly day. Day Detail opens.
S137. User modifies a day months ahead. Future override saves immediately through Day Detail.
S138. User revisits a modified future day. Saved override loads.
S139. User resets a modified future day. Override is deleted.
S140. Future override later enters Next 7 Mornings. Next 7 Mornings row hydrates saved override.
...
S154. Reset all affects immediate or near-term dates. Home, Next 7 Mornings, and scheduling refresh.
...
S174. Location change causes prayer times, Home, Next 7 Mornings, month views, and scheduling to recompute.
...
S216. Day changes at midnight. Home and Next 7 Mornings refresh to the new date context.
...
S220. User edits tomorrow from Home. Home, Next 7 Mornings, Weekly Fajrcast, monthly row, and scheduling align.
S221. User edits tomorrow from Next 7 Mornings. Home hero, Next 7 Mornings, Weekly Fajrcast, monthly row, and scheduling align.
S222. User edits tomorrow from monthly browsing. Home hero, Next 7 Mornings, Weekly Fajrcast, monthly row, and scheduling align.
S223. User edits a far-future day from monthly browsing. Monthly row and Adjusted Days update; Home/Next 7 Mornings/Weekly Fajrcast update later when the date enters their visible horizon.
...
Supporting specs: Morning Hero; Alarm Detail; Next 7 Mornings; Home Composition; Alarm Delivery Reliability
...
S230. User opens Day Detail, changes state, exits, and reopens. Saved state persists.
S231. User opens Day Detail, resets defaults, exits, and reopens. Default state persists.
S232. User modifies a future day, changes settings, revisits the day. Stored override and recomputed default are coherent.
S233. User repeatedly expands/collapses Next 7 Mornings, Browse by Month, and Weekly Fajrcast. No product state changes.
...
|---|---|---|---|
| Should `Other early worship` be visibly exposed in Home Hero and Day Detail, or preserved internally and explicitly deferred from visible MVP? | S038, S042, S092, S227 and related before-Fajr preservation scenarios | Decided for MVP: deferred from active MVP. Suhoor is the only before-Fajr wake purpose. | Quick Wake Mode Contract; Morning Hero update; Alarm Detail update |
...
| Does `Reset to Defaults` in Day Detail commit immediately or stage until `Done`? | S103, S106–S109, S231 | Decided for MVP: Day Detail edits and reset commit immediately; Done is an exit affordance. | Alarm Detail update; Wake Adjustment/Commit Contract |
...
| Next 7 Mornings | Reference Home Composition for placement/collapse context; keep compact tag suppression separate from domain meaning. |
...
```text
Review the current Subh codebase against `subh-mvp-interaction-inventory-v4.md` and `00-subh-spec-index-v3.md`. For each scenario ID S001-S235, classify implementation status as Implemented, Partially Implemented, Missing, Risky, or Not Testable Yet.
