...
## May 29 Quiet / Pause Entitlement Alignment Addendum

The following remain Free/core safety and trust controls and must not be paywalled:

```text
Quiet tomorrow / this morning
Turn alarm on from Quiet
Pause Subh wake alarms indefinitely
Resume alarms
Ring tomorrow only / this morning only while paused
I’m awake
I’m fasting today
I prayed Fajr
Core permission/setup warnings
```

Future paid features may include advanced history, reflection, recurring patterns, travel-aware planning, household coordination, or advanced analytics, but not basic alarm silence/resume/acknowledgement controls.
...
2. The exposed MVP quick wake purposes are:

```text
Fajr | Suhoor
```

3. `Suhoor` is the only exposed before-Fajr MVP wake purpose.
...
14. Wake Sessions, core Follow-up Alarms, follow-up-alarm cancellation, and active-morning awake confirmation are part of the Free core wake utility.
15. For MVP, explicit AlarmKit/system dismissal is treated as `I’m awake` while preserving acknowledgement source; pricing must not weaken the alarm acknowledgement and follow-up-alarm cancellation model.
16. `I’m awake`, `I prayed Fajr`, and `I’m fasting today` are immediate MorningLog/check-in actions when shown in the current active morning loop; they must not be treated as paid-only just because their long-term history can later feed Plus.
...
| --- | --- | --- |
| `I’m awake` | Confirms the Fajr Wake Session and cancels remaining follow-up alarms. | May later appear in Plus history, trends, or analytics. |
| `I’m awake` | Confirms the Suhoor Wake Session and cancels remaining follow-up alarms. | May later appear in Plus history, trends, or analytics. |
...
follow-up alarm scheduled/fired/stopped/cancelled
...
- confirm awake;
- cancel remaining follow-up alarms by confirming awake;
...
- Detailed Wake Session scheduling, follow-up-alarm, confirmation, and MorningLog mechanics.
...
advanced follow-up-alarm personalization if later scoped as paid
paywall routing
...
Wake Session creation and core Follow-up Alarms
immediate awake confirmation and current-morning check-ins
core follow-up-alarm cancellation after confirmation
...
- core Follow-up Alarms for resolver-supported Fajr and Suhoor wake plans;
...
| Primary alarm / follow-up-alarm event record | Yes | Yes |
| `I’m awake` | Yes | Yes |
| Cancel remaining follow-up alarms after awake confirmation | Yes | Yes |
...
Because core Suhoor, Ramadan wake planning, Wake Sessions, Follow-up Alarms, and immediate active-morning check-ins are Free, Ramadan Promo should not be used to unlock basic Suhoor wake functionality or the current morning loop. It should only affect explicitly scoped paid-layer features.
...
- wants to use Follow-up Alarms;
- wants to confirm awake for Fajr or Suhoor;
- wants to cancel remaining follow-up alarms by confirming awake;
...
| Wake Session operational records | System / all users | Yes per retention policy | Current/recent operational use; historical surfacing may be limited in Free | Required for follow-up alarms, cancellation, reconciliation, and trust. |
...
- [ ] Free can use Wake Sessions, core Follow-up Alarms, awake confirmation, and active/current-morning check-ins where implemented.
...
- [ ] Tapping Fajr, Suhoor, Quiet, wake adjustment, Follow-up Alarms, awake confirmation, active/current-morning check-in, Next 7 Mornings, or Weekly Fajrcast does not open a paywall.
...
1. `subh-wake-sessions-wake-checks-morning-logs-spec-v1.md`
   Already created as the canonical owner for Wake Sessions, Follow-up Alarms, immediate MorningLogs, and the distinction between current-morning confirmation and durable history.
...
- gate core Follow-up Alarms behind payment;
