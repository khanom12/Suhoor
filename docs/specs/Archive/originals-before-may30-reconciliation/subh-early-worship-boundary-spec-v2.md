# Subh Suhoor Boundary and Before-Fajr Window Specification
...
| Owning domain / surface | Suhoor before-Fajr boundary model; legacy early-worship terminology retained only for compatibility |
...
## May 29 Suhoor Boundary Terminology Alignment Addendum

This file retains its historical filename for compatibility, but MVP user-facing language must treat the before-Fajr wake purpose as `Suhoor`.

- `Suhoor` is the only exposed before-Fajr MVP wake purpose.
- Generic non-fasting before-Fajr, Tahajjud-only, and other early-worship purposes remain deferred.
- The relevant Suhoor follow-up alarm boundary is Fajr begins.
- The final-third boundary may remain an internal earliest-boundary calculation where supported, but visible copy must not call the MVP mode `Early`, `Pre-Fajr`, `Early worship`, or `Fast mode`.
...
For default Fajr mornings, this is the earliest meaningful wake boundary because the user’s primary objective is to wake for Fajr rather than before-Fajr worship.
...
This boundary represents the earliest meaningful time Subh should use for before-Fajr worship and preparation.
...
| Fasting intended | Final third start | `finalThirdOfNight` | Opens suhoor/before-Fajr preparation window. |
...
- it no longer supports suhoor or before-Fajr Tahajjud;
- the app should warn or explain in detail surfaces, but not overcomplicate list rows.

Recommended warning copy:

```text
This wake is after Fajr begins, so it may be too late for suhoor or before-Fajr Tahajjud.
```
...
The before-Fajr period used for fasting preparation, Tahajjud, du‘a, Qur’an, and other worship.
```
