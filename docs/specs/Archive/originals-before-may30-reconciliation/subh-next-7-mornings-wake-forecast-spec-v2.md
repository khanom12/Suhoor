...
## May 29 Next 7 Quiet / Pause Row Alignment Addendum
...
Next 7 rows must inherit the final resolved alarm state vocabulary without exposing full controls inline.

Trailing/status lane values:
...
Middle lane must not show `Quiet`, `Pause mode`, `Fajr`, or `Suhoor` as routine state tags. Wake purpose may be shown only through the row's approved purpose/status treatment if the surface design explicitly supports it without crowding opportunity/context tags.

Rows are navigation entry points to Day Detail in MVP. Do not add direct row-level Quiet/Pause toggles in this pass.

A paused row means inherited global Pause. A Quiet row means manual date-level Quiet. A `Rings once` row means a date-specific exception while global Pause remains active.
...
4. Keep row height static across all row states, especially Quiet.
5. Change the middle tag lane from a wake-state tag lane to an **opportunity/context tag lane**.
6. Remove routine state tags from the middle lane, including `Fajr`, `Fasting`, `Suhoor`, `Tahajjud`, and `Quiet`.
7. Quiet appears in the trailing status area as `Quiet`, replacing the wake time/status for that compact row.
...
- Do not show `[Quiet]` in the middle lane.
...
- Quiet must not make the row shorter.
...
- Do not add `[Quiet]`; Quiet is a trailing status.
...
Tuesday, May 5. Quiet. Double tap for details.
```
...
Do not reuse a general-purpose chip list if it forces state tags like `Fajr`, `Fasting`, or `Quiet` into this compact middle lane.
...
- Quiet does not shrink the row height.
...
- Quiet does not render `[Quiet]` in the middle lane.
- Quiet renders `Quiet` in the trailing lane.
...
15. Quiet rows do not become shorter than non-Quiet rows.
16. Quiet appears as trailing `Quiet`, not as a middle-lane tag.
...
- [ ] Fajr/Fasting/Suhoor/Tahajjud/Quiet do not appear as middle-lane tags.
