...
## May 29 Alarm Detail Quiet / Pause / Selector Alignment Addendum
...
Alarm Detail must mirror the Home Hero model for a selected editable morning:

- Wake-purpose selector: `Fajr | Suhoor` only.
- Alarm-state control: active time, `Quiet`, `Alarms paused`, `Rings this morning only`, `Turn on alarms`, or issue/setup state.
- Quiet is a selected-date alarm override and must preserve selected purpose plus Fajr/Suhoor alarm settings.
- Pause is inherited from the global policy and must be clearly distinguished from manual Quiet.
- While paused, Detail may offer `Ring this morning only`, `Resume alarms`, and `Keep paused`.
- Returning a ring-once exception to silence while Pause is active removes the exception and restores inherited Pause; it must not create manual Quiet.
- The slider is active for active/ring-once states, ghosted and not draggable for Quiet/Paused in MVP, and saved/warning-treated for blocked/setup states.
- User-facing copy must use `Alarm saved for 5:42 AM`, not `Saved wake` / `Saved Fajr wake` / `Saved Suhoor wake`.

## MVP Suhoor Alignment Addendum
This addendum is normative for MVP and supersedes conflicting lower sections in this file.

- The mode selector is `Fajr | Suhoor`.
- `Suhoor` is the only before-Fajr option exposed by Alarm Detail in MVP.
- Alarm Detail must not expose `Tahajjud only`, `Other early worship`, or a generic non-fasting before-Fajr intention selector.
...
- Explicit fasting-purpose overrides remain available as fasting-purpose choices where supported, but they do not create additional wake purposes.
...
- `Early`, `Fast`, and `Suhoor` are legacy exposed mode labels; use `Suhoor`.
...
3. Select the day’s wake purpose: `Suhoor`, `Fajr`, or `Quiet`, matching the Home hero exactly.
...
- Rename the before-Fajr wake purpose to `Suhoor`.
- Align the mode selector with the Home hero: `Fajr | Suhoor`.
...
- Ensure permission or delivery failure never becomes `Quiet`.
...
- Quiet wording and visual parity.
...
4. Primary wake time or `Quiet` state.
...
| Quiet | No wake alarm for this date | Inactive / stable region | Quiet sentence + day opportunity sentence |
...
The Quiet moon icon must remain visible exactly as it does on the Home hero. The icon must not disappear on the detail screen.
...
- The slider is active for `Suhoor` and `Fajr`.
- Quiet should preserve the same layout footprint without exposing an active slider.
...
Suhoor purpose is a wake before Fajr begins.
...
The context card must always address fasting opportunities in `Fajr`, `Suhoor`, and `Quiet` modes.
...
When relevant, a second short sentence may explain that the user can still choose `Suhoor` to plan another kind of fast:
...
### Quiet with opportunities

```text
Quiet is on for this date. No alarm will ring. This day has Sunnah fasting opportunities: [Monday fast] [White Days fast].
```

### Quiet without opportunities

```text
Quiet is on for this date. No alarm will ring. There are no Sunnah fasting opportunities for this day.
```

## 16. Pre-Fajr intention control

When `Suhoor` is selected, show a compact intention control.
...
- In Ramadan, `Suhoor` defaults to locked `Fasting` / `Ramadan fast`.
...
- On Eid days, `Fasting` is unavailable and `Suhoor` defaults to `Tahajjud only` unless a product-approved non-fasting `Other early worship` choice is explicitly selected.

### Mode switching preservation

If the user changes from `Suhoor` to `Fajr` or `Quiet`, preserve the selected Pre-Fajr intention for that date.

If the user later returns to `Suhoor`, restore the preserved intention unless Ramadan or Eid rules override it.

Manual wake-time adjustment is not preserved across mode changes. Returning to `Suhoor` uses the default Pre-Fajr wake anchor unless the user adjusts again.

## 17. Fasting-intention behavior

When the user selects `Suhoor` + `Fasting`, the context card must show the selected fasting intention.
...
- `Suhoor` defaults to `Tahajjud only`; `Other early worship` may remain available only as a non-fasting explicit choice when product rules allow it
...
- Ramadan dates default to before-Fajr waking.
- The wake anchor is Fajr begins.
- `Ramadan fast` is locked.
- The Pre-Fajr intention is locked to `Fasting` while the selected mode is `Suhoor`.
...
- `Fajr`, if they do not want a before-Fajr suhoor wake for that date.
- `Quiet`, if they do not want a wake alarm for that date.

Ramadan remains the governing day context even if the user changes the wake purpose.

If the user returns to `Suhoor`, the screen must immediately restore:

```text
Suhoor + Ramadan fast
```

The user must not be able to return to `Suhoor` during Ramadan and see `Tahajjud only`, `Other early worship`, `Voluntary fast`, Qada, Vow, Kaffarah, Other fast, or any non-Ramadan Sunnah opportunity as the selected intention.
...
If the user turns it off, the before-Fajr wake alarm remains enabled. Only the later Fajr adhan event is disabled for that date.
...
- selected wake purpose: Pre-Fajr, Fajr, Quiet
...
A committed `Other early worship` Pre-Fajr intention should be restorable after switching away from and back to `Suhoor`, unless Ramadan or Eid rules override the selected-day state.
...
- The primary wake time or `Quiet` state should be announced clearly.
- The Quiet moon icon should remain decorative unless it adds necessary meaning.
...
- The adjustment slider is present and active for `Suhoor` and `Fajr`.
- The slider behaves like the Home hero slider.
- The mode selector matches the Home hero selector.
- Quiet state displays `Quiet`.
- The Quiet moon icon remains visible in the detail hero.
...
- Suhoor purpose card copy explains whether Sunnah fasting opportunities exist.
- Quiet card copy says no alarm will ring.
...
- Selecting `Suhoor` reveals or exposes `Tahajjud only`, `Fasting`, and `Other early worship` intention options.
...
- Ramadan defaults to `Suhoor` + `Fasting` + `Ramadan fast`.
...
- Quiet is the only user-facing way to turn off the wake alarm.
...
5. Preserve the Home hero slider, selector, animations, and Quiet moon icon.
6. Add or update the always-present liquid-glass context card below the hero.
7. Remove `Use usual plan` from all card states.
8. Add mode-specific sentence copy for Fajr, deferred Tahajjud-only, deferred Other early worship, Suhoor, and Quiet.
...
12. Keep `Suhoor` intention options explicit and contract-aligned: `Tahajjud only`, `Fasting`, and `Other early worship`.
