...
Existing `MorningWakeRuleState` and `ResolvedWakeState` should remain the low-level wake model. The new layer should expose user/product-facing wake classification.
...
4. Default Fajr -> `.defaultInFajr` for in-Fajr rule, `.earlyPreFajr` only if the user’s default daily plan is pre-Fajr.
...
The system SHALL use the resolved user intention to decide whether the day uses default Fajr wake, early pre-Fajr wake, Tahajjud wake, or quiet suppression.
...
- **AND** the wake classification SHALL be early pre-Fajr unless a date-specific override applies
...
- **AND** the wake classification SHALL be early pre-Fajr
- **AND** the system SHALL NOT require fast completion for that date
...
- **AND** the wake classification SHALL be early pre-Fajr unless an explicit override applies
