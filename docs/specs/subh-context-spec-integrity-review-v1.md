...
The existing specs contain some legacy lower sections with `Pre-Fajr`, `Tahajjud only`, and `Other early worship`, but the current MVP addenda already supersede those with `Suhoor | Fajr | Quiet`.
