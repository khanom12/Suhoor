# Subh Morning State Framework Scenario Walkthrough Specification v2 — June 1 CTA and Logging Scenario Reconciliation

| Field | Value |
| --- | --- |
| Canonical filename | `subh-morning-state-framework-scenario-walkthrough-spec-v2.md` |
| Version | 2 |
...
| Fajr-start event vs optional Fajr wake session | Slider-activated Fajr wake checks after Suhoor is user-initiated and then uses normal wake checks. |
| Wake acknowledgement vs prayer completion | `I’m Awake for Fajr` confirms wake success. `I Prayed Fajr` logs prayer completion. |
| Current/previous morning logging vs next-morning hero | Late Fajr logging appears inside the context-card action area, not inside the hero after the hero has rolled forward. |
...
## 6. Late Fajr logging prompt inside the context-card action area
...
If the user has not logged Fajr prayer completion for the just-ended morning, show a separate late-logging prompt inside the context-card action area. This prompt is not part of the hero and must not confuse the next-morning plan.
...
| After Fajr end | Next-morning hero appears; unresolved Fajr prayer logging appears inside context-card action area if eligible. |
...
## 11. Suhoor completion and slider-activated Fajr wake checks after Suhoor
...
The user may intentionally choose an slider-activated Fajr wake checks after Suhoor action from the main hero, such as `Wake Me for Fajr` or `Adjust Fajr wake time`. If selected, the user switches/configures a Fajr wake session and normal wake-check rules apply.
...
- branch the simulation based on user actions such as changing purpose, dragging the slider, toggling Quiet, acknowledging Suhoor/Fajr, opting into post-Suhoor Fajr slider activation, and logging Fajr;
...
| MSF-SF-004 | Slider-activated Fajr wake checks after Suhoor is user-initiated and then uses normal wake checks. |
...
| MSF-LOG-003 | Late Fajr logging appears inside context-card action area, not in the hero after Fajr end. |
| MSF-TEST-001 | Harness supports 24/48-hour scrubbing and action-branching. |

---

## June 1 Addendum: CTA Scenario Updates

Add the following scenario families to the walkthrough:

| Scenario | Expected behaviour |
| --- | --- |
| Early Suhoor before window | Context card shows **I’m Already Awake for Suhoor**. Confirmation silences Suhoor attempts and keeps Fajr adhan/event. |
| Early Fajr before Fajr begins | Context card shows **I’m Already Awake for Fajr**. Confirmation silences Fajr adhan/alarm/checks. |
| Active alarm dismissed without awake | Current attempt stops; Hero advances to next wake-check time. |
| Active Suhoor awake | Hero transitions to same-morning Fajr and default Fajr-beginning adhan/event. |
| Post-Suhoor Fajr slider adjusted | Later slider target creates/activates Fajr wake checks. |
| Fajr awake then prayer | Hero **I’m Awake for Fajr** → cooldown → context-card **I Prayed Fajr**. |
| Late Fajr | Context-card prompt: `I prayed Fajr earlier/yesterday morning? ✓ ✕`. |
| Fast completion | Context-card prompt after Maghrib: `I completed my fast today/yesterday? ✓ ✕`. |
